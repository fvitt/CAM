...
!-----------------------------------------------------------------------
      ...
      use aurora_params, only: prescribed_period, prescribed_meped_period
      use spe_data,      only: spe_on
      use spe_data,      only: spe_eo,spe_fe, spe_fn
      ...
      logical  :: use_cion = .true.
      logical  :: use_dion = .false.

      ! g = 8.7 m/s^2? Because this is 400 km up?
      real(r8), parameter :: grav   = 870._r8          ! (cm/s^2)
!      real(r8), parameter :: gask   = 8.314e7_r8       ! gas constant
      real(r8), parameter :: boltz  = 1.38E-16_r8      ! boltzman's constant
      real(r8), parameter :: avo    = 6.023e23_r8      ! avogadro number
...
!-----------------------------------------------------------------------
      ...
      indxEfx_e = pbuf_get_index('MEPED_EFX_E', errcode=ierr)
      indxKev_e = pbuf_get_index('MEPED_KEV_E', errcode=ierr)
      indxEfx_p = pbuf_get_index('MEPED_EFX_P', errcode=ierr)
      indxKev_p = pbuf_get_index('MEPED_KEV_P', errcode=ierr)
      ...
      if (indxEfx_e>0 .and. indxEfx_p>0) then
         x_nan = nan
         call pbuf_set_field(pbuf2d, indxEfx_e, x_nan)
         call pbuf_set_field(pbuf2d, indxKev_e, x_nan)
         call pbuf_set_field(pbuf2d, indxEfx_p, x_nan)
         call pbuf_set_field(pbuf2d, indxKev_p, x_nan)
      endif
...
!-----------------------------------------------------------------------
!     plb = 5.e-4_r8*exp( 7._r8 ) * .1_r8             ! Pa at 95km
      plb = 5500._r8             ! Pa at ~20km
      do k = 1,pver
...
!-----------------------------------------------------------------------
      ...
      logical  :: do_aurora(ncol),do_spe(ncol)

      real(r8) :: dayfrac, rotation

!!$      write(6,"('aurora_prod: tn(:,1) = ',/,(10f10.1))") tn(:,1)
...
!-----------------------------------------------------------------------
      call aurora_ions( drizl, cusp, alfa, alfa2, alfa3, flux, &
                        flux2, flux3, alfa3p,flux3p, alfa_spe, flux_spe, &
                        tn, o2, o1, mbar, qo2p, qop, qn2p, &
                        qnp, pmid, do_aurora, do_spe, spe_on, ncol, lchnk, pbuf )
...
!-----------------------------------------------------------------------
      call total_ion_prod( drizl, cusp, alfa, alfa2, alfa3, &
                           ...
                           flux_spe, alfa_spe, &
                           tn, mbar, qsum, pmid, do_aurora, do_spe, spe_on,&
                           ncol )
...
!-----------------------------------------------------------------------
      ...
      logical, intent(in)     :: do_aurora(ncol), do_spe(ncol)
      type(physics_buffer_desc),pointer :: pbuf(:)
...
!-----------------------------------------------------------------------
      ...
      real(r8), pointer   :: pr_efx_e(:) ! Pointer to pbuf MEPED electron energy flux (mW m-2)
      real(r8), pointer   :: pr_kev_e(:) ! Pointer to pbuf MEPED electron mean energy (keV)
      real(r8), pointer   :: pr_efx_p(:) ! Pointer to pbuf MEPED proton energy flux (mW m-2)
      real(r8), pointer   :: pr_kev_p(:) ! Pointer to pbuf MEPED proton mean energy (keV)
      integer  :: n
...
! Define MEPED data for alfa3 and flux3
        if (prescribed_meped_period .and. indxEfx_e>0 .and. indxEfx_p>0) then
          ...
          call pbuf_get_field(pbuf, indxEfx_e, pr_efx_e)
          call pbuf_get_field(pbuf, indxKev_e, pr_kev_e)
          call pbuf_get_field(pbuf, indxEfx_p, pr_efx_p)
          call pbuf_get_field(pbuf, indxKev_p, pr_kev_p)

         do n=1,ncol
          alfa3(n) = max(pr_kev_e(n),30._r8)/2._r8
          flux3(n) = max(pr_efx_e(n)/(2._r8*alfa3(n)*1.602e-9_r8),1.e-20_r8)
! Convert keV to Mev for MEPED proton
          alfa3p(n) = max(pr_kev_p(n)*1.e-3_r8,30.e-3_r8)
! Convert from mW/m2/s to MeV/cm2/s
          flux3p(n) = max(pr_efx_p(n)/1.602e-6_r8,1.e-20_r8)
         enddo
        endif
!
      where( do_spe(:) )
...
! spe_fe is in MeV/cm2/s
      ...
      subroutine aurora_ions( drizl, cusp, alfa1, alfa2, alfa3, flux1, &
                              flux2, flux3, alfa3p,flux3p, alfa_spe, flux_spe, &
                              tn, o2, o1, mbar, qo2p, qop, qn2p, &
                              qnp, pmid, do_aurora, do_spe, spe_on, ncol, lchnk, pbuf )
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
      ...
      logical, intent(in) :: do_aurora(ncol), do_spe(ncol), spe_on
...
!         endif
!
! Solar energetic protons - SPE
!         write(6,*)'Before calling bion for spe: k = ', k
!         write(6,*)'Before calling bion for spe: spe_on, do_spe = ',spe_on, do_spe
!         write(6,*)'Before calling bion for spe: do_aurora = ',do_aurora
!!$       write(6,"('Before calling bion for spe: p0ez = ',/,(10g10.3))") p0ez
!!$       write(6,"('Before calling bion for spe: tn(:,k) = ',/,(10g10.3))") tn(:,k)
!!$       write(6,"('Before calling bion for spe: tempi = ',/,(10g10.3))") tempi
!!$       write(6,"('Before calling bion for spe: barm = ',/,(10g10.3))") barm
!!$       write(6,"('Before calling bion for spe: barm_t = ',/,(10g10.3))") barm_t
!!$       write(6,"('Before calling bion for spe: tk_mbar = ',/,(10g10.3))") tk_mbar
!!$       write(6,"('Before calling bion for spe: alfa_spe = ',/,(10g10.3))") alfa_spe
!!$       write(6,"('Before calling bion for spe: xalfa_spe = ',/,(10g10.3))") xalfa_spe
!!$
...
!         if (masterproc .and. k>50 ) then
!      write(6,"('After calling bion for spe: k = ',i3))") k
!      write(6,"('After calling bion for spe: press = ',/,(10g10.3))") press
!      write(6,"('After calling bion for spe: tempi = ',/,(10g10.3))") tempi
!      write(6,"('After calling bion for spe: barm = ',/,(10g10.3))") barm
!      write(6,"('After calling bion for spe: tk_mbar = ',/,(10g10.3))") tk_mbar
!      write(6,"('After calling bion for spe: p0ez_mbar = ',/,(10g10.3))") p0ez_mbar
!      write(6,"('After calling bion for spe: alfa_spe = ',/,(10g10.3))") alfa_spe
!      write(6,"('After calling bion for spe: xalfa_spe = ',/,(10g10.3))") xalfa_spe
!      write(6,"('After calling bion for spe: spe_bion = ',/,(10g10.3))") spe_bion
!         endif
!
...
!      write(6,"('After calling bion for meped: xalfa1 = ',/,(10g10.3))") xalfa1
!      write(6,"('After calling bion for meped: falfa1 = ',/,(10g10.3))") falfa1
!      write(6,"('After calling bion for meped: xalfa3 = ',/,(10g10.3))") xalfa3
!      write(6,"('After calling bion for meped: flux3 = ',/,(10g10.3))") flux3
!      write(6,"('After calling bion for meped: falfa3 = ',/,(10g10.3))") falfa3
!      write(6,"('After calling bion for meped: xalfa3p = ',/,(10g10.3))") xalfa3p
!      write(6,"('After calling bion for meped: flux3p = ',/,(10g10.3))") flux3p
!      write(6,"('After calling bion for meped: falfa3p = ',/,(10g10.3))") falfa3p
!      write(6,"('After calling bion for meped: alfa1_ion = ',/,(10g10.3))") alfa1_ion
!      write(6,"('After calling bion for meped: alfa3_ion = ',/,(10g10.3))") alfa3_ion
!      write(6,"('After calling bion for meped: alfa3p_bion = ',/,(10g10.3))") alfa3p_bion
!      write(6,"('After calling bion for meped: qmeped_e = ',/,(10g10.3))") qmeped_e
!      write(6,"('After calling bion for meped: qmeped_p = ',/,(10g10.3))") qmeped_p
!         endif
...
!-----------------------------------------------------------------------
      ...
      subroutine total_ion_prod( drizl, cusp, alfa1, alfa2, alfa3, &
                           ...
                           flux_spe, alfa_spe, &
                           tn, mbar, tpions, pmid, do_aurora, do_spe, spe_on, &
                           ncol )
!-----------------------------------------------------------------------
! 	... calculate auroral additions to ionization rates
!-----------------------------------------------------------------------
