...
! Private module data
   ...
   ! BGC carbon cycle switch
   ! if TRUE don't override advected CO2, yet have prescribed CO2 available for
   ! radiatively passive cases (set via pre-determined lower boundary CO2)
   logical, public, protected :: carbon_cycle = .FALSE.
...
subroutine chem_surfvals_readnl(nlfile)
   ...
   namelist /chem_surfvals_nl/ co2vmr, n2ovmr, ch4vmr, f11vmr, f12vmr, &
                               ...
                               ghg_yearStart_model, ghg_yearStart_data, carbon_cycle
   ...
   call mpi_bcast(carbon_cycle, 1, mpi_logical, mstrid, mpicom, ierr)
   if (ierr /= 0) call endrun(sub//" mpi_bcast: carbon_cycle")
   ...
   if (masterproc) then
      ...
      write(iulog,*) '  carbon_cycle = ',carbon_cycle
...
!-----------------------------------------------------------------------
   ...
   if (scenario_ghg == 'FIXED') then
      ...
      call flbc_inti( flbc_file, flbc_list, flbc_timing, co2vmr, ch4vmr, n2ovmr, f11vmr, f12vmr, carbon_cycle )
      call chem_surfvals_set()
