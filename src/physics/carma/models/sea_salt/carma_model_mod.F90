    ...
    if (shortname .eq. "SALT") then
      ...
      do icol = 1,ncol
        ...
        if (cam_in%ocnfrac(icol) > 0._r8) then
          ...
          !**********************************
          call CARMAMODEL_SurfaceWind(carma, state, icol, ilat(icol), ilon(icol), cam_in, u10in, rc)
...
!          if (do_print) write(LUNOPRT, *) "ncflx = " , ncflx, ", surfaceFlux = ", surfaceFlux(icol)
  ...
  !! @author  Tianyi Fan
  !! @version August-2010
  subroutine CARMAMODEL_SurfaceWind(carma, state, icol, ilat, ilon, cam_in, u10in, rc)
    use ppgrid,           only: pcols, pver
    use physics_types,    only: physics_state
    use camsrfexch,       only: cam_in_t
