...
contains
  ...
  subroutine mosaic_cam_init
    ...
    use module_data_mosaic_aero,      only: nbin_a_max, nbin_a, mhyst_method, mhyst_force_up, &
         ...
         ih2so4_g, ihno3_g, ihcl_g, inh3_g, imsa_g, ilim2_g, ilim1_g, iaro1_g, iapi1_g, iapi2_g
    ...
    alpha_ASTEM     = 0.5_r8  !Solver parameter. range: 0.01 - 1.0
    rtol_eqb_ASTEM  = 0.01_r8 !Relative eqb tolerance. range: 0.01 - 0.03
    ptol_mol_ASTEM  = 0.01_r8 !Percent mol tolerance.  range: 0.01 - 1.0
...
! soag currently only condenses onto modes 1-3
    ...
    do ibin = 1, ntot_amode
       ...
       if (lptr2_soa_a_amode(ibin,2) > 0 .or. lgtmpa) i_gas2bin_uptk_flag(ilim1_g, ibin) = 1
       if (lptr2_soa_a_amode(ibin,3) > 0 .or. lgtmpa) i_gas2bin_uptk_flag(iaro1_g, ibin) = 1
       if (lptr2_soa_a_amode(ibin,4) > 0 .or. lgtmpa) i_gas2bin_uptk_flag(iapi1_g, ibin) = 1
       if (lptr2_soa_a_amode(ibin,5) > 0 .or. lgtmpa) i_gas2bin_uptk_flag(iapi2_g, ibin) = 1
    end do
    if ( masterproc ) then
       ...
       write(iulog,'(a,10i5)') 'api2  ', i_gas2bin_uptk_flag(iapi2_g, 1:ntot_amode)
