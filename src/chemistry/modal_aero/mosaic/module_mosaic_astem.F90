...
! use module_print_aer,  only: print_aer
  ...
  complex(r8),  intent(out), dimension(nbin_a_max) :: ri_shell_a,ri_avg_a,ri_core_a
...
10 mosaic_vars_aa%isteps_ASTEM = mosaic_vars_aa%isteps_ASTEM + 1
  ...
  do 501 ibin = 1, nbin_a
     ...
     do iv = 1, ngas_ioa
        sfc_a(iv)                  = gas(iv)
        df_gas_s(iv,ibin)          = 0.0_r8
        df_gas_l(iv,ibin)          = 0.0_r8
...
! update estimated possible condensation for each bin - used to calculate "tiny" amounts
     if(jaerosolstate(ibin) .ne. no_aerosol)then
        delta_nh3_max(ibin) = 0.1_r8*gas(inh3_g)*kgfrac_nh3(ibin)
        delta_hno3_max(ibin)= 0.1_r8*gas(ihno3_g)*kgfrac_hno3(ibin)
        delta_hcl_max(ibin) = 0.1_r8*gas(ihcl_g)*kgfrac_hcl(ibin)
     endif
! RAZ 2/2/2015: end bugfix
...
! check if extremely slow inter-particle mass transfer is occurring after 20 ASTEM steps
  ...
  if(mosaic_vars_aa%isteps_ASTEM .gt. 20)then
    ...
    do iv = 2, 4 ! HNO3, HCl, NH3
      ...
      if(gas(iv) .gt. 0.01_r8 .and. sum_phi_volatile(iv) .gt. 0.01_r8)islow_intermassxfer = mNO
  ...
  do 20 iv = 2, 4
     ...
     do 21 ibin = 1, nbin_a
        ...
        sum1 = sum1 + aer(iv,jp,ibin)/   &
             (1._r8 + dtmax*kg(iv,ibin)*Heff(iv,ibin)*integrate(iv,jp,ibin))
     ...
     do 22 ibin = 1, nbin_a
        ...
        if(integrate(iv,jliquid,ibin) .eq. mYES)then
           aer(iv,jliquid,ibin) =   &
                ...
                (1._r8 + dtmax*kg(iv,ibin)*Heff(iv,ibin))
        endif
...
40 continue
  ...
  if(t_new .lt. 0.9999_r8*t_out) goto 10
...
30 mosaic_vars_aa%cumul_steps_ASTEM = mosaic_vars_aa%cumul_steps_ASTEM + mosaic_vars_aa%isteps_ASTEM
  ...
  do ibin = 1, nbin_a
     ...
     if(jaerosolstate(ibin) .eq. mixed)then
        if( electrolyte(jnh4no3,jsolid,ibin).gt. 0.0_r8 .or.   &
             electrolyte(jnh4cl, jsolid,ibin).gt. 0.0_r8 )then
           call ASTEM_flux_mix(ibin, phi_nh4no3_s, phi_nh4cl_s, ieqblm_ASTEM,        &
                idry_case3a, sfc_a, df_gas_s, df_gas_l, jaerosolstate, flux_s, Heff,    &
...
subroutine ASTEM_calculate_dtmax( dtchem, dtmax, jaerosolstate, idry_case3a,       &
  ...
  h_sub_max = dtchem/5.0_r8	! sec RAZ 2/14/2014
  ...
  do 5 iv = 2, ngas_ioa
     ...
     sumflux_s = 0.0_r8
     do ibin = 1, nbin_a
        if(flux_s(iv,ibin) .gt. 0.0_r8)then
           sumflux_s = sumflux_s + flux_s(iv,ibin)
        endif
     enddo
     
     if(sumflux_s .gt. 0.0_r8)then
        h_gas_i(iv) = 0.1_r8*gas(iv)/sumflux_s
...
5 continue
  ...
  do 6 iv = 2, ngas_ioa
     h_gas_i(iv) = 1.e16_r8
     sum_kg_phi = 0.0_r8
     sum_kg_phi_pos = 0.0_r8
     sum_kg_phi_neg = 0.0_r8
     do ibin = 1, nbin_a
        if(integrate(iv,jliquid,ibin) .eq. mYES)then
...
! RAZ 2/2/2015: revised algorithm: begin
          if(phi_volatile_l(iv,ibin) .gt. 0.0_r8)then
...
6 continue
  ...
  do iv = 2, ngas_ioa
     ...
     do ibin = 1, nbin_a
        if(flux_s(iv,ibin) .lt. 0._r8)then	! aer -> gas
           sum_bin_s(iv) = sum_bin_s(iv) + 1.0_r8
     ...
     if(sum_vol_s(iv) .gt. 0.0_r8)then
        avg_df_gas_s(iv) = sum_vdf_s(iv)/sum_vol_s(iv)
     else
        avg_df_gas_s(iv) = 1.0_r8 ! never used, but set to 1.0 just to be safe
     endif
  ...
  do 20 ibin = 1, nbin_a
     ...
     do 10 iv = 2, ngas_ioa
        ...
        if(flux_s(iv,ibin) .lt. 0._r8)then				! aer -> gas
           ...
           if(idry_case3a(ibin) .eq. mYES)alpha = 1.0_r8
...
!! end new algorithm - 6/3/2015 RAZ
    ...
    if(aH2O*100._r8 .lt. MDRH(ibin)) then
       jaerosolstate(ibin) = all_solid
       jphase(ibin) = jsolid
       call adjust_solid_aerosol(ibin,jphase,aer,jhyst_leg,electrolyte,epercent,water_a)
       return
...
subroutine ASTEM_flux_wet(ibin, ieqblm_ASTEM, sfc_a, df_gas_s, df_gas_l,              &
  ...
  if(water_a(ibin) .eq. 0.0_r8)then
  ...
  if(electrolyte(jcaco3,jsolid,ibin) .gt. 0.0_r8)then
...
! if(XT.lt.1.9999 .and. XT.ge.0.)then  ! RAZ 11/10/2014
  if(XT.lt.2.0_r8 .and. XT.ge.0._r8)then  ! RAZ 11/10/2014
  ...
  if( (gas(inh3_g)+aer(inh4_a,jliquid,ibin)) .lt. 1.e-25_r8)goto 10  ! no ammonia in the system
  ...
  if(g_nh3_hno3 .gt. 0._r8 .and. a_nh4_no3 .eq. 0._r8)then
  ...
  if(g_nh3_hcl .gt. 0._r8 .and. a_nh4_cl .eq. 0._r8)then
...
10 iadjust = mNO		! default
  ...
  ! hno3
  if(gas(ihno3_g).gt.0._r8 .and. aer(ino3_a,jliquid,ibin).eq.0._r8 .and.   &
       aer(icl_a,jliquid,ibin) .gt. 0.0_r8)then
  ...
  ! hcl
  if(gas(ihcl_g).gt.0._r8 .and. aer(icl_a,jliquid,ibin) .eq. 0._r8 .and.   &
       aer(ino3_a,jliquid,ibin) .gt. 0.0_r8)then
...
subroutine ASTEM_flux_wet_case1(ibin,ieqblm_ASTEM,sfc_a,df_gas_s,flux_s,         &
  ...
  ! same as dry case1
  if(gas(ihno3_g) .gt. 1.e-6_r8)then
     sfc_a(ihno3_g) = 0.0_r8
     df_gas_s(ihno3_g,ibin) = gas(ihno3_g)
     phi_volatile_s(ihno3_g,ibin) = 1.0_r8
  ...
  if(gas(ihcl_g) .gt. 1.e-6_r8)then
     sfc_a(ihcl_g)  = 0.0_r8
     df_gas_s(ihcl_g,ibin) = gas(ihcl_g)
     phi_volatile_s(ihcl_g,ibin) = 1.0_r8
...
subroutine ASTEM_flux_wet_case2(ibin,ieqblm_ASTEM,sfc_a,df_gas_l,Heff,           &
  ...
  if(dum_hno3 .gt. 0.0_r8)then
     Heff(ihno3_g,ibin)=   &
          kel(ihno3_g,ibin)*gam(jhno3,ibin)**2*mc(jc_h,ibin)*1.e-9_r8/   &
  ...
  if(dum_hcl .gt. 0.0_r8)then
     Heff(ihcl_g,ibin)=   &
          kel(ihcl_g,ibin)*gam(jhcl,ibin)**2*mc(jc_h,ibin)*1.e-9_r8/   &
  ...
  if(dum_nh3 .gt. 0.0_r8)then
     Heff(inh3_g,ibin) =   &
          kel(inh3_g,ibin)*gam_ratio(ibin)*1.e-9_r8*Keq_ll(3)/   &
...
subroutine ASTEM_flux_wet_case3(ibin,ieqblm_ASTEM,sfc_a,df_gas_l,Heff,           &
  ...
  sfc_a(ihno3_g) = Keq_nh4no3/max(sfc_a(inh3_g),1.e-20_r8)
  sfc_a(ihcl_g)  = Keq_nh4cl/max(sfc_a(inh3_g),1.e-20_r8)
  
  
  ! diagnose mH+
  if(sfc_a(ihno3_g).gt.0.0_r8 .and. ma(ja_no3,ibin).gt.0.0_r8)then
     mc(jc_h,ibin) = Keq_gl(3)*sfc_a(ihno3_g)/   &
          (kel(ihno3_g,ibin)*gam(jhno3,ibin)**2 * ma(ja_no3,ibin))
  elseif(sfc_a(ihcl_g).gt.0.0_r8 .and. ma(ja_cl,ibin).gt.0.0_r8)then
...
subroutine ASTEM_flux_wet_case3b(ibin,ieqblm_ASTEM,sfc_a,df_gas_l,Heff,          &
  ...
  ! diagnose mH+
  if(sfc_a(ihcl_g).gt.0.0_r8 .and. ma(ja_cl,ibin).gt.0.0_r8)then
     mc(jc_h,ibin) = Keq_gl(4)*sfc_a(ihcl_g)/   &
          (kel(ihcl_g,ibin)*gam(jhcl,ibin)**2 * ma(ja_cl,ibin))
  else
     mc(jc_h,ibin) = sqrt(Keq_ll(3))
...
subroutine ASTEM_flux_wet_case4(ibin,ieqblm_ASTEM,sfc_a,df_gas_l,Heff,           &
  ...
  if(dum_denom .eq. 0.0_r8 .or. dum_numer .eq. 0.0_r8)then
     mc(jc_h,ibin) = sqrt(Keq_ll(3))
     return
  endif
...
! if(XT.lt.1.9999 .and. XT.ge.0.)then   ! excess sulfate (acidic) ! RAZ 11/10/2014
  if(XT.lt.2.0_r8 .and. XT.ge.0._r8)then   ! excess sulfate (acidic) ! RAZ 11/10/2014
  ...
  if(volatile_cl .gt. 0.0_r8 .and. gas(ihno3_g).gt. 0.0_r8 )then
  ...
  if(prod_nh4no3 .gt. 0.0_r8 .or. prod_nh4cl .gt. 0.0_r8)then
...
subroutine ASTEM_flux_dry_case2(ibin,ieqblm_ASTEM,sfc_a,df_gas_s,flux_s,         &
  ...
  if(gas(inh3_g).gt.1.e-6_r8)then
     sfc_a(inh3_g) = 0.0_r8
     df_gas_s(inh3_g,ibin) = gas(inh3_g)
     phi_volatile_s(inh3_g,ibin)  = 1.0_r8
...
subroutine ASTEM_flux_dry_case3b(ibin, phi_nh4cl_s,ieqblm_ASTEM,sfc_a,df_gas_s,  &
  ...
  epercent(jnh4cl,jsolid,ibin) = 100._r8*electrolyte(jnh4cl,jsolid,ibin)/sum_dum
  !! EFFI
  ...
  elseif(gas(inh3_g)*gas(ihcl_g) .lt. Keq_sg(2) .and.   &
     ...
     iactive_nh4cl = 0
     if(epercent(jnh4cl, jsolid,ibin) .gt. 0.0_r8)then
  ...
  if(flux_s(ihcl_g,ibin) .gt. 0.0_r8)then
...
subroutine ASTEM_flux_dry_case4(ibin, phi_nh4no3_s,phi_nh4cl_s,ieqblm_ASTEM,     &
  ...
  epercent(jnh4no3,jsolid,ibin) = 100._r8*electrolyte(jnh4no3,jsolid,ibin)/sum_dum
  epercent(jnh4cl, jsolid,ibin) = 100._r8*electrolyte(jnh4cl, jsolid,ibin)/sum_dum
  !! EFFI
  ...
  elseif(gas(inh3_g)*gas(ihno3_g) .lt. Keq_sg(1) .and.   &
     ...
     iactive_nh4no3 = 0
     if(epercent(jnh4no3,jsolid,ibin) .gt. 0.0_r8)then
...
subroutine ASTEM_flux_mix(ibin, phi_nh4no3_s, phi_nh4cl_s, ieqblm_ASTEM, idry_case3a, &
  ...
  epercent(jcaco3,jsolid,ibin) = 100._r8*electrolyte(jcaco3,jsolid,ibin)/sum_dum
  !! EFFI
  ...
  if(epercent(jcaco3,jsolid,ibin) .gt. 0.0_r8)then
     jphase(ibin) = jliquid
     call ASTEM_flux_wet_case1(ibin,ieqblm_ASTEM,sfc_a,df_gas_s,flux_s,          &
          phi_volatile_s,integrate,jphase,kg,gas,mc,Keq_ll)
     return
...
! if(XT.lt.1.9999 .and. XT.ge.0.)then   ! excess sulfate (acidic) ! RAZ 11/10/2014
  ...
  if( electrolyte(jnh4no3,jsolid,ibin).gt.0._r8 .and.   &
       electrolyte(jnh4cl,jsolid,ibin) .gt.0._r8 )then
     jphase(ibin) = jsolid
     call ASTEM_flux_dry_case4(ibin,phi_nh4no3_s,phi_nh4cl_s,ieqblm_ASTEM,sfc_a, &
          df_gas_s,flux_s,phi_volatile_s,integrate,kg,gas,electrolyte,epercent,  &
          Keq_sg,aer)
  ...
  elseif( electrolyte(jnh4no3,jsolid,ibin).gt.0._r8 )then
     ! do small adjustments for nh4cl aq
     g_nh3_hcl= gas(inh3_g)*gas(ihcl_g)
     a_nh4_cl = aer(inh4_a,jliquid,ibin)*aer(icl_a,jliquid,ibin)

     iadjust = mNO              ! initialize
     if(g_nh3_hcl .gt. 0.0_r8 .and. a_nh4_cl .eq. 0.0_r8)then
        call absorb_tiny_nh4cl(ibin,aer,gas,electrolyte,delta_nh3_max,           &
             delta_hcl_max,electrolyte_sum)
        iadjust = mYES
     elseif(g_nh3_hcl .eq. 0.0_r8 .and. a_nh4_cl .gt. 0.0_r8)then
  ...
  elseif( electrolyte(jnh4cl,jsolid,ibin).gt.0._r8)then
     ! do small adjustments for nh4no3 aq
     g_nh3_hno3= gas(inh3_g)*gas(ihno3_g)
     a_nh4_no3 = aer(inh4_a,jliquid,ibin)*aer(ino3_a,jliquid,ibin)

     iadjust = mNO              ! initialize
     if(g_nh3_hno3 .gt. 0.0_r8 .and. a_nh4_no3 .eq. 0.0_r8)then
        call absorb_tiny_nh4no3(ibin,aer,gas,electrolyte,delta_nh3_max,          &
             delta_hno3_max,electrolyte_sum)
        iadjust = mYES
     elseif(g_nh3_hno3 .eq. 0.0_r8 .and. a_nh4_no3 .gt. 0.0_r8)then
...
subroutine ASTEM_flux_mix_case4a(ibin, phi_nh4no3_s,ieqblm_ASTEM,sfc_a,df_gas_s, &
  ...
  ! nh4cl aq
  if( gas(inh3_g)*gas(ihcl_g).eq.0._r8 .or. Keq_nh4cl.eq.0._r8 )then
     iactive_nh4cl = mNO
  endif
...
subroutine ASTEM_flux_mix_case4b(ibin,phi_nh4cl_s,ieqblm_ASTEM,sfc_a,df_gas_s,   &
  ...
  ! nh4no3 (liquid)
  if( gas(inh3_g)*gas(ihno3_g).eq.0._r8 .or. Keq_nh4no3.eq.0._r8 )then
     iactive_nh4no3 = mNO
  endif
  ...
  if(iactive_nh4cl .eq. mYES)then
     ...
     if(sfc_a(ihcl_g).gt.0.0_r8 .and. ma(ja_cl,ibin).gt.0.0_r8)then
        mc(jc_h,ibin) = Keq_gl(4)*sfc_a(ihcl_g)/   &
             (kel(ihcl_g,ibin)*gam(jhcl,ibin)**2 * ma(ja_cl,ibin))
     elseif(sfc_a(ihno3_g).gt.0.0_r8 .and. ma(ja_no3,ibin).gt.0.0_r8)then
        mc(jc_h,ibin) = Keq_gl(3)*sfc_a(ihno3_g)/   &
             (kel(ihno3_g,ibin)*gam(jhno3,ibin)**2 * ma(ja_no3,ibin))
     else
        mc(jc_h,ibin) = sqrt(Keq_ll(3))
...
! debug output (Remove this BALLI after debugging)
  ...
  ! MSA
  if(gas(imsa_g) .gt. 1.e-14_r8)then

     ! integrate msa condensation analytically
     decay_msa   = exp(-sumkg_msa*dtchem)
     delta_tmsa  = gas(imsa_g)*(1.0_r8 - decay_msa)
  ...
  delta_nh3 = gas(inh3_g) *(1.0_r8 - exp(-sumkg_nh3*dtchem))
  delta_hno3= gas(ihno3_g)*(1.0_r8 - exp(-sumkg_hno3*dtchem))
  delta_hcl = gas(ihcl_g) *(1.0_r8 - exp(-sumkg_hcl*dtchem))
  ...
  if(delta_h2so4 .eq. 0.0_r8 .and. delta_tmsa .eq. 0.0_r8)then
  ...
  do ibin = 1, nbin_a
     ...
     if(electrolyte(jnacl,jtotal,ibin)  .eq. 0.0_r8 .and.   &
          electrolyte(jcacl2,jtotal,ibin) .eq. 0.0_r8 .and.   &
          electrolyte(jnano3,jtotal,ibin) .eq. 0.0_r8 .and.   &
          electrolyte(jcano3,jtotal,ibin) .eq. 0.0_r8 .and.   &
          electrolyte(jcaco3,jtotal,ibin) .eq. 0.0_r8 .and.   &
          jaerosolstate(ibin) .ne. no_aerosol)then

        delta_nh4(ibin) = min( (2._r8*delta_so4(ibin)+delta_msa(ibin)),   &
             delta_nh3_max(ibin) )
...
!-----------------------------------------------------------------------
 subroutine ASTEM_flux_soa(ibin,sfc_a,Heff,integrate,aer,gas,sat_soa,ieqblm_soa, &
  ...
  small_oc  = 1.e-15_r8		! ng/m^3
  
  
  ! default fluxes and other stuff
  do iv = isoa_first, ngas_volatile
     sfc_a(iv)               = gas(iv)
     df_gas_o(iv,ibin)       = 0.0_r8
     flux_o(iv,ibin)         = 0.0_r8
     phi_volatile_o(iv,ibin) = 0.0_r8
  enddo
  ...
  if (use_cam5mam_soa_params > 0) then
     sum_soa = sum_soa + aer(ioc_a,jp,ibin)*0.25_r8/150._r8  ! assume 25% of OC=POA is OOA, with MW=150
  else
     sum_soa = sum_soa + aer(ioc_a,jp,ibin)/200._r8  ! 200 is assumed MW of primary OC
  end if
  
  
  ! check threshold concentration for SOA formation in the absence of primary OC
  if(aer(ioc_a,jp,ibin) .eq. 0.0_r8)then
     sum_dum = 0.0_r8
     do iv = isoa_first, ngas_volatile
        sum_dum = sum_dum + (gas(iv)+aer(iv,jp,ibin))/sat_soa(iv)
     enddo
     
     if(sum_dum .le. 1.0_r8)then	! transfer all aer to gas and quit
        do iv = isoa_first, ngas_volatile
           gas(iv)         = gas(iv) + aer(iv,jp,ibin)
           aer(iv,jp,ibin) = 0.0_r8
           integrate(iv,jp,ibin) = 0.0_r8
        enddo
        return
     endif
     
     sum_soa = max(sum_soa, 1.e-10_r8)
