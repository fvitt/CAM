...
! define error tolerances become stricter as aH2O approaches 1.0
    ...
    mass_dry_a(ibin) = 0.0_r8          ! initialize to 0.0
    ...
    vol_dry_a(ibin)  = vol_dry_a(ibin)*1.e-15_r8                               ! cc(aer)/cc(air) or m^3/m^3(air)

    ! wet mass and wet volume
    mass_wet_a(ibin) = mass_dry_a(ibin) + water_a(ibin)*1.e-3_r8               ! g/cc(air)
    vol_wet_a(ibin)  = vol_dry_a(ibin) + water_a(ibin)*1.e-3_r8                ! cc(aer)/cc(air) or m^3/m^3(air)
...
90  if (iter_kelvin_state == 200) then
    ...
    do iv = 1,  ngas_aerchtot
       term = 4._r8*sigma_soln(ibin)*partial_molar_vol(iv)/   &
            (8.3144e7_r8*T_K*DpmV(ibin))
       kel(iv,ibin) = 1._r8 + term*(1._r8 + 0.5_r8*term*(1._r8 + term/3._r8))
    enddo
  ...
  subroutine MESA( ibin, jaerosolstate, jphase, aer, jhyst_leg,      &
    ...
    if(sum_dum .eq. 0._r8)sum_dum = 1.0_r8

    do je = 1, nelectrolyte
       epercent(je,jtotal,ibin) = 100._r8*electrolyte(je,jtotal,ibin)/sum_dum
    enddo
...
!! begin new algorithm - 6/3/2015 RAZ
    ...
    if( (epercent(jcano3,jtotal,ibin) .gt. ptol_mol_astem) .or. &
      ...
      CRH = 0.0_r8  ! no crystrallization or efflorescence point
    else
  ...
  ! now diagnose MDRH
    ...
    elseif(XT .lt. 1._r8 .and. XT .gt. 0.0_r8)then  	! excess sulfate, always liquid, MDRH=0.0
       MDRH(ibin) = 0.0_r8
    elseif(XT .ge. 2.0_r8 .or. XT .lt. 0.0_r8)then  	! sulfate poor
    ...
    CRH = min(CRH, MDRH(ibin)/100.0_r8)		! 6/3/2015 RAZ
...
!! end new algorithm - 6/3/2015 RAZ
    ...
    elseif (mhyst_method == mhyst_uporlo_waterhyst) then ! 3-D method/logic
       if (water_a_hyst(ibin) > 0.5_r8*water_a_up(ibin)) jdum = 1
...
!
          ...
          water_a(ibin) = aerosol_water(jtotal,ibin,jaerosolstate,jphase,jhyst_leg,   &
               electrolyte,aer,kappa_nonelectro,num_a,mass_dry_a,mass_soluble_a,aH2O,molality0)
          if(water_a(ibin) .le. 0.0_r8)then     ! one last attempt to catch bad input
             jdum = 0
...
!       endif
    ...
    if(aH2O_a(ibin)*100._r8 .lt. MDRH(ibin)) then
       jaerosolstate(ibin) = all_solid
       jphase(ibin) = jsolid
       jhyst_leg(ibin) = jhyst_lo
       call adjust_solid_aerosol(ibin,jphase,aer,jhyst_leg,electrolyte,epercent,water_a)
...
10  call do_full_deliquescence(ibin,aer,electrolyte)
  ...
  subroutine calculate_kelvin(ibin,num_a,vol_wet_a,aH2O_a,DpmV,kelvin,sigma_soln,  &
    ...
    DpmV(ibin)=(6._r8*volume_a(ibin)/(num_a(ibin)*pi))**(1._r8/3._r8)        ! [cm]
    ...
    sigma_soln(ibin) = sigma_water + 49.0_r8*(1._r8 - aH2O_a(ibin))       ! [dyn/cm]



    term = 72._r8*sigma_soln(ibin)/(8.3144e7_r8*T_K*DpmV(ibin))           ! [-]
!    kelvin(ibin) = exp(term)
    kelvin(ibin) = 1._r8 + term*(1._r8 + 0.5_r8*term*(1._r8 + term/3._r8))
  ...
  subroutine calculate_XT(ibin,jp,XT,aer)
    ...
    if( (aer(iso4_a,jp,ibin)+aer(imsa_a,jp,ibin)) .gt.0.0_r8)then
       XT   = ( aer(inh4_a,jp,ibin) +   &
            aer(ina_a,jp,ibin)  +   &
            2._r8*aer(ica_a,jp,ibin) )/   &
            (aer(iso4_a,jp,ibin)+0.5_r8*aer(imsa_a,jp,ibin))
    else
       XT   = -1.0_r8
    endif
  ...
  subroutine adjust_liquid_aerosol(ibin,jphase,aer,jhyst_leg,electrolyte,epercent) ! TOUCH
    ...
    do je = 1, nelectrolyte
       ...
       epercent(je,jsolid,ibin)     = 0.0_r8
    ...
    electrolyte(jcaco3,jliquid,ibin)= 0.0_r8
    electrolyte(jcaso4,jliquid,ibin)= 0.0_r8
    epercent(jcaco3,jliquid,ibin)   = 0.0_r8
    epercent(jcaso4,jliquid,ibin)   = 0.0_r8
    ...
    aer(ino3_a,jsolid,ibin) = 0.0_r8
    aer(icl_a,jsolid,ibin)  = 0.0_r8
    aer(inh4_a,jsolid,ibin) = 0.0_r8
    aer(imsa_a,jsolid,ibin) = 0.0_r8
    aer(ina_a,jsolid,ibin)  = 0.0_r8
...
!   aer(ilim2_a,jliquid,ibin)= 0.0
  ...
  subroutine do_full_deliquescence(ibin,aer,electrolyte)    ! TOUCH
    ...
    do js = 1, nelectrolyte
       electrolyte(js,jsolid,ibin)  = 0.0_r8
...
!   aer(ilim2_a,jliquid,ibin)= 0.0
  ...
  subroutine MESA_PTC(ibin, jaerosolstate, jphase, aer, jhyst_leg,  &
    ...
    do js = 1, nsalt
       jsalt_present(js) = 0                        ! default value - salt absent
       if(epercent(js,jtotal,ibin) .gt. 1.0_r8)then
          jsalt_present(js) = 1                     ! salt present
       endif
    enddo
    ...
    mass_dry_salt(ibin) = 0.0_r8               ! soluble salts only
    do je = 1, nsalt
       mass_dry_salt(ibin) = mass_dry_salt(ibin) +   &
            electrolyte(je,jtotal,ibin)*mw_electrolyte(je)*1.e-15_r8   ! g/cc(air)
    enddo
    ...
    do 500 itdum = 1, mosaic_vars_aa%Nmax_MESA
       ...
       do js = 1, nsalt
          ...
          if(itdum .gt. 1 .and. phi_prod .gt. 0.0_r8)then
             phi_bar(js) = (abs(phi_salt(js))-abs(phi_salt_old(js)))/   &
                  alpha_salt(js)
          else
             phi_bar(js) = 0.0_r8                      ! oscillating, or phi_salt and/or phi_salt_old may be zero
          endif

          if(phi_bar(js) .lt. 0.0_r8)then              ! good. phi getting lower. maybe able to take bigger alphas
             phi_bar(js) = max(phi_bar(js), -10.0d0)
             alpha_fac = 3.0_r8*exp(phi_bar(js))
             alpha_salt(js) = min(alpha_fac*abs(phi_salt(js)), 0.9d0)
          elseif(phi_bar(js) .gt. 0.0_r8)then  ! bad - phi is getting bigger. so be conservative with alpha
          ...
          elseif(flux_sl(js) .lt. 0._r8)then
...
500 continue     ! end time continuation loop
  ...
  subroutine MESA_flux_salt(ibin, jaerosolstate,jphase,aer,jhyst_leg,electrolyte,  &
    ...
    activity(jna3hso4,ibin)   = 0.0_r8

    if(water_a(ibin) .le. 0.0_r8)then
       do js = 1, nsalt
          flux_sl(js) = 0.0_r8
       enddo
       return
    ...
    do je = 1, nelectrolyte
       epercent(je,jliquid,ibin) = 100._r8*electrolyte(je,jliquid,ibin)/sum_dum
    enddo
    !! EFFI
    ...
    if(sum_salt .eq. 0.0_r8)sum_salt = 1.0_r8
    do js = 1, nsalt
       frac_salt_solid(js) = electrolyte(js,jsolid,ibin)/sum_salt
       frac_salt_liq(js)   = epercent(js,jliquid,ibin)/100._r8
    enddo
    ...
    do js = 1, nsalt             ! soluble solid salts
       ...
       phi_salt(js)  = (sat_ratio(js) - 1.0_r8)/max(sat_ratio(js),1.0d0)

       ! check if too little solid-phase salt is trying to dissolve
       if(sat_ratio(js)       .lt. 1.00_r8 .and.   &
            frac_salt_solid(js) .lt. 0.01_r8 .and.   &
            frac_salt_solid(js) .gt. 0.0_r8)then
       ...
       ! compute flux
       flux_sl(js) = sat_ratio(js) - 1.0_r8

       ! apply Heaviside function
       if( (sat_ratio(js)               .lt. 1.0_r8 .and.   &
            electrolyte(js,jsolid,ibin) .eq. 0.0_r8) .or.   &
            (calcium .gt. 0.0_r8 .and. frac_salt_liq(js).lt.0.01_r8).or.   &
            (calcium .gt. 0.0_r8 .and. jsalt_present(js).eq.0) )then
          flux_sl(js) = 0.0_r8
          phi_salt(js)= 0.0_r8
       endif
    ...
    sat_ratio(jcano3) = 1.0_r8
    phi_salt(jcano3)  = 0.0_r8
    flux_sl(jcano3)   = 0.0_r8

    sat_ratio(jcacl2) = 1.0_r8
    phi_salt(jcacl2)  = 0.0_r8
    flux_sl(jcacl2)   = 0.0_r8
 ...
 !***********************************************************************
  ...
  subroutine compute_activities(ibin,jaerosolstate,jphase,aer,jhyst_leg,           &
    ...
    if(water_a(ibin) .eq. 0.0_r8)return


    call calculate_XT(ibin,jliquid,XT,aer)


    if(XT.ge.2.0_r8 .or. XT.lt.0._r8)then   ! changed .gt. to .ge.   RAZ 4/16/2014
       ! SULFATE POOR: fully dissociated electrolytes


       ! anion molalities (mol/kg water)
       ma(ja_so4,ibin)  = 1.e-9_r8*aer(iso4_a,jliquid,ibin)/water_a(ibin)
       ma(ja_hso4,ibin) = 0.0_r8
       ma(ja_no3,ibin)  = 1.e-9_r8*aer(ino3_a,jliquid,ibin)/water_a(ibin)
       ma(ja_cl,ibin)   = 1.e-9_r8*aer(icl_a, jliquid,ibin)/water_a(ibin)
       ma(ja_msa,ibin)  = 1.e-9_r8*aer(imsa_a,jliquid,ibin)/water_a(ibin)

       ! cation molalities (mol/kg water)
       mc(jc_ca,ibin)   = 1.e-9_r8*aer(ica_a, jliquid,ibin)/water_a(ibin)
       mc(jc_nh4,ibin)  = 1.e-9_r8*aer(inh4_a,jliquid,ibin)/water_a(ibin)
       mc(jc_na,ibin)   = 1.e-9_r8*aer(ina_a, jliquid,ibin)/water_a(ibin)
       a_c              = (   &
            (2._r8*ma(ja_so4,ibin)+   &
            ma(ja_no3,ibin)+   &
            ma(ja_cl,ibin) +   &
            ma(ja_msa,ibin)) -   &
            (2._r8*mc(jc_ca,ibin) +   &
       ...
       mc(jc_h,ibin) = 0.5_r8*( (a_c) +   &
            (sqrt(a_c**2 + 4._r8*Keq_ll(3))) )

       if(mc(jc_h,ibin) .le. 0.0_r8)then   ! changed .eq. to .le. RAZ 4/16/2014
          !mc(jc_h,ibin) = 1.e-10_r8
       ...
       sum_elec = 2._r8*electrolyte(jnh4no3,jp,ibin) +   &
            2._r8*electrolyte(jnh4cl,jp,ibin)  +   &
            3._r8*electrolyte(jnh4so4,jp,ibin) +   &
            3._r8*electrolyte(jna2so4,jp,ibin) +   &
            2._r8*electrolyte(jnano3,jp,ibin)  +   &
            2._r8*electrolyte(jnacl,jp,ibin)   +   &
            3._r8*electrolyte(jcano3,jp,ibin)  +   &
            3._r8*electrolyte(jcacl2,jp,ibin)  +   &
            2._r8*electrolyte(jhno3,jp,ibin)   +   &
            2._r8*electrolyte(jhcl,jp,ibin)

       if(sum_elec .eq. 0.0_r8)then
       ...
       xmol(jnh4no3) = 2._r8*electrolyte(jnh4no3,jp,ibin)/sum_elec
       xmol(jnh4cl)  = 2._r8*electrolyte(jnh4cl,jp,ibin) /sum_elec
       xmol(jnh4so4) = 3._r8*electrolyte(jnh4so4,jp,ibin)/sum_elec
       xmol(jna2so4) = 3._r8*electrolyte(jna2so4,jp,ibin)/sum_elec
       xmol(jnano3)  = 2._r8*electrolyte(jnano3,jp,ibin) /sum_elec
       xmol(jnacl)   = 2._r8*electrolyte(jnacl,jp,ibin)  /sum_elec
       xmol(jcano3)  = 3._r8*electrolyte(jcano3,jp,ibin) /sum_elec
       xmol(jcacl2)  = 3._r8*electrolyte(jcacl2,jp,ibin) /sum_elec
       xmol(jhno3)   = 2._r8*electrolyte(jhno3,jp,ibin)  /sum_elec
       xmol(jhcl)    = 2._r8*electrolyte(jhcl,jp,ibin)   /sum_elec


       jA = jnh4so4
       if(xmol(jA).gt.0.0_r8)then
...
!      endif
       ...
       !----
10     gam(jlvcite,ibin) = 1.0_r8

       gam(jnh4hso4,ibin)= 1.0_r8

       gam(jnh4msa,ibin) = 1.0_r8

       gam(jna3hso4,ibin) = 1.0_r8

       gam(jnahso4,ibin) = 1.0_r8

       gam(jnamsa,ibin)  = 1.0_r8

       gam(jcamsa2,ibin) = 1.0_r8

       activity(jlvcite,ibin) = 0.0_r8

       activity(jnh4hso4,ibin)= 0.0_r8
       ...
       sum_elec = 3._r8*electrolyte(jh2so4,jp,ibin)    +   &
            2._r8*electrolyte(jnh4hso4,jp,ibin)  +   &
            5._r8*electrolyte(jlvcite,jp,ibin)   +   &
            3._r8*electrolyte(jnh4so4,jp,ibin)   +   &
            2._r8*electrolyte(jnahso4,jp,ibin)   +   &
            5._r8*electrolyte(jna3hso4,jp,ibin)  +   &
            3._r8*electrolyte(jna2so4,jp,ibin)   +   &
            2._r8*electrolyte(jhno3,jp,ibin)     +   &
            2._r8*electrolyte(jhcl,jp,ibin)
       ...
       xmol(jh2so4)  = 3._r8*electrolyte(jh2so4,jp,ibin)/sum_elec
       xmol(jnh4hso4)= 2._r8*electrolyte(jnh4hso4,jp,ibin)/sum_elec
       xmol(jlvcite) = 5._r8*electrolyte(jlvcite,jp,ibin)/sum_elec
       xmol(jnh4so4) = 3._r8*electrolyte(jnh4so4,jp,ibin)/sum_elec
       xmol(jnahso4) = 2._r8*electrolyte(jnahso4,jp,ibin)/sum_elec
       xmol(jna3hso4)= 5._r8*electrolyte(jna3hso4,jp,ibin)/sum_elec
       xmol(jna2so4) = 3._r8*electrolyte(jna2so4,jp,ibin)/sum_elec
       xmol(jhno3)   = 2._r8*electrolyte(jhno3,jp,ibin)/sum_elec
       xmol(jhcl)    = 2._r8*electrolyte(jhcl,jp,ibin)/sum_elec
...
20     gam(jnh4no3,ibin) = 1.0_r8
       gam(jnh4cl,ibin)  = 1.0_r8
       gam(jnano3,ibin)  = 1.0_r8
       gam(jnacl,ibin)   = 1.0_r8
       gam(jcano3,ibin)  = 1.0_r8
       gam(jcacl2,ibin)  = 1.0_r8
       ...
       ! anion molalities (mol/kg water)
       mSULF            = 1.e-9_r8*aer(iso4_a,jliquid,ibin)/water_a(ibin)
       ma(ja_hso4,ibin) = 0.0_r8
       ma(ja_so4,ibin)  = 0.0_r8
       ma(ja_no3,ibin)  = 1.e-9_r8*aer(ino3_a,jliquid,ibin)/water_a(ibin)
       ma(ja_cl,ibin)   = 1.e-9_r8*aer(icl_a, jliquid,ibin)/water_a(ibin)
       ma(ja_msa,ibin)  = 1.e-9_r8*aer(imsa_a,jliquid,ibin)/water_a(ibin)
       ...
       c_bal =  mc(jc_nh4,ibin) + mc(jc_na,ibin) + 2._r8*mc(jc_ca,ibin)   &
       ...
       !--quadratic solution
       if(bq .ne. 0.0_r8)then
          xq = 4._r8*(1._r8/bq)*(cq/bq)
       else
          xq = 1.e+6_r8
       endif

       if(abs(xq) .lt. 1.e-6_r8)then
          dum = xq*(0.5_r8 + xq*(0.125_r8 + xq*0.0625_r8))
          quad = (-0.5_r8*bq/aq)*dum
          if(quad .lt. 0._r8)then
             quad = -bq/aq - quad
          endif
       else
          quad = 0.5_r8*(-bq+sqrt(bq*bq - 4._r8*cq))
       endif
       !--end of quadratic solution

       mc(jc_h,ibin) = max(quad, 1.E-7_r8)
       ...
       activity(jnh4no3,ibin) = 0.0_r8

       activity(jnh4cl,ibin)  = 0.0_r8

       activity(jnano3,ibin)  = 0.0_r8

       activity(jnacl,ibin)   = 0.0_r8

       activity(jcano3,ibin)  = 0.0_r8

       activity(jcacl2,ibin)  = 0.0_r8
  ...
  subroutine MESA_convergence_criterion(ibin,iconverge_mass,iconverge_flux,        &
    ...
    mass_solid_salt = 0.0_r8
    do je = 1, nsalt
       mass_solid_salt = mass_solid_salt +   &
            electrolyte(je,jsolid,ibin)*mw_electrolyte(je)*1.e-15_r8        ! g/cc(air)
    enddo
    ...
    if(mass_dry_salt(ibin) .le. 0.0_r8)then
      frac_solid = 0.0_r8
    else
    ...
    if(sumflux .eq. 0.0_r8 .and. crustal_solids .eq. 0.0_r8)then
       idissolved = myes
    endif
  ...
  subroutine electrolytes_to_ions(jp,ibin,aer,electrolyte)
    ...
    aer(imsa_a,jp,ibin) = electrolyte(jnh4msa,jp,ibin) +   &
         ...
         2._r8*electrolyte(jcamsa2,jp,ibin) +   &
    ...
    aer(ina_a,jp,ibin)  = electrolyte(jnano3,jp,ibin)  +   &
         ...
         2._r8*electrolyte(jna2so4,jp,ibin) +   &
         3._r8*electrolyte(jna3hso4,jp,ibin)+   &
         electrolyte(jnahso4,jp,ibin) +   &
         electrolyte(jnamsa,jp,ibin)

    aer(inh4_a,jp,ibin) = electrolyte(jnh4no3,jp,ibin) +   &
         electrolyte(jnh4cl,jp,ibin)  +   &
         2._r8*electrolyte(jnh4so4,jp,ibin) +   &
         3._r8*electrolyte(jlvcite,jp,ibin) +   &
         electrolyte(jnh4hso4,jp,ibin)+   &
...
!    if(XT.ge.1.9999 .or. XT.lt.0.)then ! commented out by RAZ 4/16/2014
    if(XT.ge.2.0_r8 .or. XT.lt.0._r8)then     ! Slightly different logic, consistent with that in compute_activities subr. RAZ 4/16/2014
    ...
    if(icase.eq.1)then ! XT >= 2 or XT < 0: SULFATE POOR (OR NO SULFATE) DOMAIN. RAZ 4/16/2014
       ...
       cat_net = (   &
            (2._r8*na(ja_so4)+na(ja_no3)+na(ja_cl)+na(ja_msa)) -   &
            (2._r8*nc(jc_ca) +nc(jc_nh4)+nc(jc_na)) )

       if(cat_net .lt. 0.0_r8)then
       ...
       if(sum_naza .eq. 0._r8 .or. sum_nczc .eq. 0._r8)then ! it's ok. this may happen if the aerosol is assumed to be composed of hygroscopic SOA, POA, BC, OIN, but does not contain any inorganic electrolytes
...
!         call mosaic_warn_mess( tmp_str )   ! write message
    ...
    elseif(icase.eq.2)then ! XT < 2 : SULFATE RICH DOMAIN
       ...
       if(sum_na_nh4 .gt. store(imsa_a))then
          ...
          rem_nh4 = 0.0_r8  ! remaining nh4
          rem_na  = 0.0_r8  ! remaining na
       endif


       ! recompute XT
       if(aer(iso4_a,jp,ibin).gt.0.0_r8)then
          XT = (rem_nh4 + rem_na)/aer(iso4_a,jp,ibin)
       else
          goto 10
       endif

       if(XT .le. 1.0_r8)then            ! h2so4 + bisulfate
       ...
       elseif(XT .le. 1.5_r8)then    ! bisulfate + letovicite
...
10  sum_dum = 0.0_r8
  ...
  subroutine MESA_estimate_eleliquid(ibin,XT,aer,electrolyte,zc,za,xeq_a,na_Ma,    &
    ...
    if(XT .ge. 2.0_r8 .or. XT.lt.0._r8)then
    ...
    if(icase.eq.1)then ! XT >= 2 : SULFATE POOR DOMAIN
       ...
       cano3   = min(dum_ca, 0.5_r8*dum_no3)
       dum_ca  = max(0.d0, dum_ca - cano3)
       dum_no3 = max(0.d0, dum_no3 - 2._r8*cano3)

       cacl2   = min(dum_ca, 0.5_r8*dum_cl)
       dum_ca  = max(0.d0, dum_ca - cacl2)
       dum_cl  = max(0.d0, dum_cl - 2._r8*cacl2)
       ...
       cat_net = (   &
            (2._r8*na(ja_so4)+na(ja_no3)+na(ja_cl)+na(ja_msa)) -   &
            (2._r8*nc(jc_ca) +nc(jc_nh4)+nc(jc_na)) )   ! RAZ 11/11/2014: bug fix. remove nc(jc_h)
       ...
       if(sum_naza .eq. 0._r8 .or. sum_nczc .eq. 0._r8)then
    ...
    elseif(icase.eq.2)then ! XT < 2 : SULFATE RICH DOMAIN
       ...
       if(sum_na_nh4 .gt. store(imsa_a))then
          ...
          store(inh4_a)= 0.0_r8  ! remaining nh4
          store(ina_a) = 0.0_r8  ! remaining na
       endif

       if(store(iso4_a).eq.0.0_r8)goto 10

       XT_d  = XT
       XNa_d = 1._r8 + 0.5_r8*store(ina_a)/store(iso4_a)
       xdum = store(iso4_a) - store(inh4_a)

       dum = ( (2._r8*store(iso4_a)) -   &
            (store(ina_a)) )
       if(store(inh4_a) .gt. 0.0_r8 .and. dum .gt. 0.0_r8)then
          XNH4_d = 2._r8*store(inh4_a)/   &
               (2._r8*store(iso4_a) - store(ina_a))
       else
          XNH4_d = 0.0_r8
       endif


       IF(store(inh4_a) .gt. 0.0_r8)THEN
          if(XT_d .ge. XNa_d)then
             eleliquid(jna2so4) = 0.5_r8*store(ina_a)

             if(XNH4_d .ge. 5._r8/3._r8)then
                eleliquid(jnh4so4) = 1.5_r8*store(ina_a)   &
                     - 3._r8*xdum - store(inh4_a)
                eleliquid(jlvcite) = 2._r8*xdum + store(inh4_a)   &
                     - store(ina_a)
             elseif(XNH4_d .ge. 1.5_r8)then
                eleliquid(jnh4so4) = store(inh4_a)/5._r8
                eleliquid(jlvcite) = store(inh4_a)/5._r8
             elseif(XNH4_d .ge. 1.0_r8)then
                eleliquid(jnh4so4) = store(inh4_a)/6._r8
                eleliquid(jlvcite) = store(inh4_a)/6._r8
                eleliquid(jnh4hso4)= store(inh4_a)/6._r8
             endif

          elseif(XT_d .gt. 1.0_r8)then
             eleliquid(jnh4so4)  = store(inh4_a)/6._r8
             eleliquid(jlvcite)  = store(inh4_a)/6._r8
             eleliquid(jnh4hso4) = store(inh4_a)/6._r8
             eleliquid(jna2so4)  = store(ina_a)/3._r8
             eleliquid(jnahso4)  = store(ina_a)/3._r8
          elseif(XT_d .le. 1.0_r8)then
             eleliquid(jna2so4)  = store(ina_a)/4._r8
             eleliquid(jnahso4)  = store(ina_a)/2._r8
             eleliquid(jlvcite)  = store(inh4_a)/6._r8
             eleliquid(jnh4hso4) = store(inh4_a)/2._r8
          endif

       ELSE

          if(XT_d .gt. 1.0_r8)then
             eleliquid(jna2so4) = store(ina_a) - store(iso4_a)
             eleliquid(jnahso4) = 2._r8*store(iso4_a) -   &
                  store(ina_a)
          else
             eleliquid(jna2so4) = store(ina_a)/4._r8
             eleliquid(jnahso4) = store(ina_a)/2._r8
...
10  return
  ...
  subroutine MESA_dissolve_small_salt(ibin,js,aer,electrolyte)
    ...
    if(js .eq. jnh4so4)then
       aer(inh4_a,jliquid,ibin) = aer(inh4_a,jliquid,ibin) +   &
            2._r8*electrolyte(js,jsolid,ibin)
       aer(iso4_a,jliquid,ibin) = aer(iso4_a,jliquid,ibin) +   &
            electrolyte(js,jsolid,ibin)
    ...
    if(js .eq. jlvcite)then
       aer(inh4_a,jliquid,ibin) = aer(inh4_a,jliquid,ibin) +   &
            3._r8*electrolyte(js,jsolid,ibin)
       aer(iso4_a,jliquid,ibin) = aer(iso4_a,jliquid,ibin) +   &
            2._r8*electrolyte(js,jsolid,ibin)
  ...
  subroutine form_camsa2(store,jp,ibin,electrolyte)
    ...
    electrolyte(jcamsa2,jp,ibin) = min(store(ica_a),0.5_r8*store(imsa_a))
    store(ica_a)  = ( (store(ica_a)) -   &
         (electrolyte(jcamsa2,jp,ibin)) )
  ...
  subroutine aerosolmtc( jaerosolstate, num_a, Dp_wet_a, sigmag_a, P_atm, T_K, aH2O, aer, kg )
    ...
    use module_data_mosaic_aero,  only: r8, nbin_a_max, nbin_a, naer, naercomp,             &!Parameters
         ...
         !iaro1_g,iaro2_g,ialk1_g, iapi1_g,iapi2_g, ilim1_g
    ...
    parameter (tworootpi = 3.5449077_r8, root2 = 1.4142135_r8, beta = 2.0_r8)
    ...
    tmpa = 0.1_r8
    if ( use_cam5mam_accom_coefs > 0 ) tmpa = 0.65_r8
...
!   accom(inh3_g)    = tmpa
!   accom(imsa_g)    = tmpa
!   accom(iaro1_g)   = 0.2_r8
!   accom(iaro2_g)   = 0.1_r8
!   accom(ialk1_g)   = 0.1_r8
!   accom(iole1_g)   = tmpa
!   accom(iapi1_g)   = 0.015_r8 !(11->0.1, 12->0.08, 13-> 0.05, 14,15-> 0.025)
!   accom(iapi2_g)   = 0.00025_r8 !(11->0.1, 12-> 0.01, 13-> 0.005, 14,15->0.002)
!   accom(ilim1_g)   = 0.1_r8
!   accom(ilim2_g)   = 1.0_r8
!   accom(iaro1_g)   = tmpa ! RAZ: 3/14/2019
!   accom(iaro2_g)   = tmpa
!   accom(ialk1_g)   = tmpa
!   accom(iole1_g)   = tmpa
!   accom(iapi1_g)   = tmpa
...
!zlu soa update accomdation coef. for SOA VBS


    ! quadrature weights
    xghq(1) =  0.70710678_r8
    xghq(2) = -0.70710678_r8
    wghq(1) =  0.88622693_r8
    wghq(2) =  0.88622693_r8
    ...
    do iv = 1, ngas_ioa
       ...
       freepath(iv) = 3._r8*Dg(iv)/speed                    ! cm
    enddo
...
! het-rct gases
    ...
    if (mSIZE_FRAMEWORK .eq. mMODAL) then
       ...
       do 10 ibin = 1, nbin_a
          ...
          lnDpgn = log(Dp_wet_a(ibin)) - 1.5_r8*lnsg*lnsg

          cdum   = tworootpi*num_a(ibin)*   &
               exp(beta*lnDpgn + 0.5_r8*(beta*lnsg)**2)


! RH-dependent accommodation coefficient for HNO3 on dust particles (Li et al. ACP, 12, 7591-7607, 2012).
            if(aer(ica_a,jtotal,ibin) .gt. 0._r8)then ! dust particles
              if(aH2O .lt. 0.8_r8)then
                accom(ihno3_g)   = (0.0018_r8*aH2O)/((1.0_r8-aH2O)*(1.0_r8 + 7.0_r8*aH2O))
                accom(ihno3_g)   = max( accom(ihno3_g), 1.8e-5_r8 )  ! avoid accom = 0 which causes problems
                    accom(ihcl_g)    = accom(ihno3_g)
              else
                accom(ihno3_g)   = 0.0011_r8 ! max value capped at 0.0011
                accom(ihcl_g)    = 0.0011_r8
              endif
...
10     continue
    ...
    elseif ((mSIZE_FRAMEWORK .eq. mSECTIONAL   ) .or. &
       ...
       do 11 ibin = 1, nbin_a
          ...
          cdum  = 6.283185_r8*Dp_wet_a(ibin)*num_a(ibin)

! RH-dependent accommodation coefficient for HNO3 on dust particles (Li et al. ACP, 12, 7591-7607, 2012).
	    if(aer(ica_a,jtotal,ibin) .gt. 0._r8)then ! dust particles
    	      if(aH2O .lt. 0.8_r8)then
                !accom(ihno3_g)   = (0.018_r8*8.0_r8*aH2O)/((1.0_r8-aH2O)*(1.0_r8-(1.0_r8-8.0_r8)*aH2O))
                accom(ihno3_g)   = (0.0018_r8*aH2O)/((1.0_r8-aH2O)*(1.0_r8 + 7.0_r8*aH2O))
                accom(ihno3_g)   = max( accom(ihno3_g), 1.8e-5_r8 )  ! avoid accom = 0 which causes problems
	            accom(ihcl_g)    = accom(ihno3_g)
              else
                !accom(ihno3_g)   = 0.09_r8 ! max value capped at 0.09.
                accom(ihno3_g)   = 0.0011_r8 ! max value capped at 0.0011
	        accom(ihcl_g)    = 0.0011_r8
              endif
            else	! non-dust particles
          ...
          do 21 iv = 1, ngas_aerchtot
             Kn = 2._r8*freepath(iv)/Dp_wet_a(ibin)
             Fkn = fuchs_sutugin(Kn,accom(iv))
...
11     continue
  ...
  subroutine calc_dry_n_wet_aerosol_props(                          &
    ...
    use module_data_mosaic_constants,  only: pi,piover6,third
    ...
    complex(r8), intent(inout), dimension(nbin_a_max) :: ri_avg_a,ri_core_a,ri_shell_a
    ! local variables
    integer i, iaer, isize, itype, j, jc, je, k
    real(r8) :: aer_H, duma, vol_core, vol_shell, vol_dum
    real(r8),dimension(naercomp) :: comp_a
    complex(r8) rixvol_tot, rixvol_core, rixvol_shell


    ! calculate dry mass and dry volume of a bin
    mass_dry_a(ibin) = 0.0_r8                ! initialize to 0.0
    vol_dry_a(ibin)  = 0.0_r8                ! initialize to 0.0
    area_dry_a(ibin) = 0.0_r8                ! initialize to 0.0
    ...
    if(jaerosolstate(ibin) .ne. no_aerosol)then
       ...
       ! wet mass and wet volume
       mass_wet_a(ibin) = mass_dry_a(ibin) + water_a(ibin)*1.e-3_r8  ! g/cc(air)
       vol_wet_a(ibin)  = vol_dry_a(ibin) + water_a(ibin)*1.e-3_r8   ! cc(aer)/cc(air)
       ...
       area_dry_a(ibin)= pi*num_a(ibin)*Dp_dry_a(ibin)**2    ! cm^2/cc(air)
       area_wet_a(ibin)= pi*num_a(ibin)*Dp_wet_a(ibin)**2    ! cm^2/cc(air)
       ...
       do je = 1, nelectrolyte
          comp_a(je)=electrolyte(je,jtotal,ibin)*mw_comp_a(je)*1.e-15_r8     ! g/cc(air)
       enddo
       comp_a(joc)  = aer(ioc_a,  jtotal,ibin)*mw_comp_a(joc  )*1.e-15_r8    ! g/cc(air)
       comp_a(jbc)  = aer(ibc_a,  jtotal,ibin)*mw_comp_a(jbc  )*1.e-15_r8    ! g/cc(air)
       comp_a(join) = aer(ioin_a, jtotal,ibin)*mw_comp_a(join )*1.e-15_r8    ! g/cc(air)
...
!      comp_a(jlim2)= aer(ilim2_a,jtotal,ibin)*mw_comp_a(jlim2)*1.e-15    ! g/cc(air)
       ...
       j = jsoa_first + k - 1
       i = isoa_first + k - 1
       comp_a(j) = aer(i,jtotal,ibin)*mw_comp_a(j)*1.e-15_r8    ! g/cc(air)
       end do

       comp_a(jh2o) = water_a(ibin)*1.e-3_r8                         ! g/cc(air)

       rixvol_tot   = (0.0_r8,0.0_r8)
       do jc = 1, naercomp
       ...
       ri_core_a(ibin)  = (0.0_r8,0.0_r8)
       Dp_core_a(ibin)  = 0.0_r8
       ...
       dens_dry_a(ibin) = 1.0_r8      ! g/cc(aerosol)
       dens_wet_a(ibin) = 1.0_r8      ! g/cc(aerosol)
       ...
       ri_avg_a(ibin) = (1.5_r8,0.0_r8)
       ri_shell_a(ibin) = (1.5_r8,0.0_r8)
       ri_core_a(ibin)  = (0.0_r8,0.0_r8)
       Dp_core_a(ibin)  = 0.0_r8
...
!   if(XT .ge. 1.9999 .or. XT.lt.0.)then
    if(XT .ge. 2.0_r8 .or. XT.lt.0._r8)then         ! RAZ 11/10/2014
...
10     if(jp .eq. jsolid)then
   ...
   subroutine form_na2so4(store,jp,ibin,electrolyte)
    ...
    real(r8), intent(inout), dimension(nelectrolyte,3,nbin_a_max) :: electrolyte
    electrolyte(jna2so4,jp,ibin) = min(.5_r8*store(ina_a),   &
         store(iso4_a))
  ...
  subroutine form_cano3(store,jp,ibin,electrolyte)        ! Ca(NO3)2
    ...
    electrolyte(jcano3,jp,ibin) = min(store(ica_a),0.5_r8*store(ino3_a))
  ...
  subroutine form_cacl2(store,jp,ibin,electrolyte)
    ...
    electrolyte(jcacl2,jp,ibin) = min(store(ica_a),0.5_r8*store(icl_a))
  ...
  subroutine form_nacl(store,jp,ibin,aer,gas,electrolyte,total_species,tot_cl_in)
    ...
    if(store(icl_a) .lt. 0._r8)then                          ! cl deficit in aerosol. take some from gas
       ...
       store(icl_a) = 0._r8                                          ! force store(icl_a) to 0.
  ...
  subroutine form_nh4so4(store,jp,ibin,electrolyte)       ! (nh4)2so4
    ...
    electrolyte(jnh4so4,jp,ibin)= min(.5_r8*store(inh4_a),   &
         store(iso4_a))
  ...
  subroutine form_nh4so4_lvcite(store,jp,ibin,electrolyte) ! (nh4)2so4 + (nh4)3h(so4)2
    ...
    electrolyte(jnh4so4,jp,ibin)= ( (2._r8*store(inh4_a)) -   &
         (3._r8*store(iso4_a)) )
    electrolyte(jlvcite,jp,ibin)= ( (2._r8*store(iso4_a)) -   &
         (store(inh4_a)) )
  ...
  subroutine form_lvcite_nh4hso4(store,jp,ibin,electrolyte) ! (nh4)3h(so4)2 + nh4hso4
    ...
    electrolyte(jlvcite,jp,ibin) = ( (store(inh4_a)) -   &
         (store(iso4_a)) )
    electrolyte(jnh4hso4,jp,ibin)= ( (3._r8*store(iso4_a)) -   &
         (2._r8*store(inh4_a)) )
    electrolyte(jlvcite,jp,ibin) = max(0.d0,   &
         electrolyte(jlvcite,jp,ibin))
  ...
  subroutine form_na2so4_nahso4(store,jp,ibin,electrolyte) ! na2so4 + nahso4
    ...
    electrolyte(jna2so4,jp,ibin)= ( (store(ina_a)) -   &
         (store(iso4_a)) )
    electrolyte(jnahso4,jp,ibin)= ( (2._r8*store(iso4_a))-   &
         (store(ina_a)) )
 ...
 subroutine degas_hno3(store,jp,ibin,aer,gas,electrolyte)
  ...
  subroutine absorb_tiny_nh4no3(ibin,aer,gas,electrolyte,delta_nh3_max,            &
    ...
    !! EFFI
    electrolyte_sum(jtotal,ibin) = 0.0_r8
    do je = 1, nelectrolyte
    ...
    small_gas = 0.01_r8 * min(delta_nh3_max(ibin),delta_hno3_max(ibin))
    small_aer = 0.01_r8 * electrolyte_sum(jtotal,ibin)
    if(small_aer .eq. 0.0_r8)small_aer = small_gas
  ...
  subroutine absorb_tiny_nh4cl(ibin,aer,gas,electrolyte,delta_nh3_max,             &
    ...
    small_gas = 0.01_r8 * min(delta_nh3_max(ibin), delta_hcl_max(ibin))
    small_aer = 0.01_r8 * electrolyte_sum(jtotal,ibin)
    if(small_aer .eq. 0.0_r8)small_aer = small_gas
  ...
  subroutine absorb_tiny_hno3(ibin,aer,gas,delta_hno3_max)        ! and degas tiny hcl
    ...
    small_gas = 0.01_r8 * delta_hno3_max(ibin)
    small_aer = 0.01_r8 * aer(icl_a,jliquid,ibin)
  ...
  subroutine absorb_tiny_hcl(ibin,aer,gas,delta_hcl_max)  ! and degas tiny hno3
    ...
    small_gas = 0.01_r8 * delta_hcl_max(ibin)
    small_aer = 0.01_r8 * aer(ino3_a,jliquid,ibin)
  ...
  subroutine degas_tiny_nh4no3(ibin,aer,gas,electrolyte)
    ...
    small_amt = 0.01_r8 * electrolyte(jnh4no3,jliquid,ibin)
  ...
  subroutine degas_tiny_nh4cl(ibin,aer,gas,electrolyte)
    ...
    small_amt = 0.01_r8 * electrolyte(jnh4cl,jliquid,ibin)
  ...
  subroutine equilibrate_hcl(ibin,aer,gas,electrolyte,activity,mc,water_a,         &
    ...
    aerSO4 = ma(ja_so4,ibin)*water_a(ibin)*1.e+9_r8
    aerHSO4= ma(ja_hso4,ibin)*water_a(ibin)*1.e+9_r8

    Tcl = aer(icl_a,jliquid,ibin) + gas(ihcl_g)   ! nmol/m^3(air)
    Kdash_hcl = Keq_gl(4)*1.e+18_r8/gam(jhcl,ibin)**2        ! (nmol^2/kg^2)/(nmol/m^3(air))
    Z = (   aer(ina_a, jliquid,ibin) +               &  ! nmol/m^3(air)
         aer(inh4_a,jliquid,ibin) +   &
         2._r8*aer(ica_a, jliquid,ibin) ) -   &
         (2._r8*aerSO4  +   &
         aerHSO4 +   &
         aer(ino3_a,jliquid,ibin) )
    ...
    Kdash_hcl = Keq_gl(4)*1.e+18_r8/gam(jhcl,ibin)**2        ! (nmol^2/kg^2)/(nmol/m^3(air))
    a = 1.0_r8
    b = ((Kdash_hcl*W) + (Z/W))*1.e-9_r8
    c = Kdash_hcl*(Z - Tcl)*1.e-18_r8


    dum = ((b*b)-(4._r8*a*c))
    if (dum .lt. 0._r8) return               ! no real root
    ...
    ma(ja_so4,ibin)  = 1.e-9_r8*aerSO4/water_a(ibin)
    ma(ja_hso4,ibin) = 1.e-9_r8*aerHSO4/water_a(ibin)
    ma(ja_no3,ibin)  = 1.e-9_r8*aer(ino3_a,jliquid,ibin)/water_a(ibin)
    ma(ja_cl,ibin)   = 1.e-9_r8*aer(icl_a, jliquid,ibin)/water_a(ibin)
  ...
  subroutine equilibrate_hno3(ibin,aer,gas,electrolyte,activity,mc,water_a,        &
    ...
    Kdash_hno3 = Keq_gl(3)*1.e+18_r8/gam(jhno3,ibin)**2      ! (nmol^2/kg^2)/(nmol/m^3(air))
    Z = (   aer(ina_a, jliquid,ibin) +               &  ! nmol/m^3(air)
         aer(inh4_a,jliquid,ibin) +   &
         2._r8*aer(ica_a, jliquid,ibin) ) -   &
         (2._r8*aerSO4  +   &
    ...
    Kdash_hno3 = Keq_gl(3)*1.e+18_r8/gam(jhno3,ibin)**2      ! (nmol^2/kg^2)/(nmol/m^3(air))
    a = 1.0_r8
    b = ((Kdash_hno3*W) + (Z/W))*1.e-9_r8
    c = Kdash_hno3*(Z - Tno3)*1.e-18_r8
  ...
  subroutine equilibrate_hcl_and_hno3(ibin,aer,gas,electrolyte,activity,mc,        &
    ...
    p = (Z/W + W*(Kdash_hcl + Kdash_hno3))*1.e-9_r8

    q = 1.e-18_r8*Kdash_hcl*Kdash_hno3*W**2  +   &
         1.e-18_r8*Z*(Kdash_hcl + Kdash_hno3) -   &
         1.e-18_r8*Kdash_hcl*Tcl -   &
         1.e-18_r8*Kdash_hno3*Tno3

    r = 1.e-18_r8*Kdash_hcl*Kdash_hno3*W*(Z - Tcl - Tno3)*1.e-9_r8

    mH = cubic(p,q,r)

    if(mH .gt. 0.0_r8)then
       aerH = mH*W*1.e+9_r8
...
10     call degas_hno3(store,jp,ibin,aer,gas,electrolyte)
  ...
  function cubic( psngl, qsngl, rsngl )
    ...
    if(D .gt. 0._r8)then     !       => 1 real and 2 complex roots
       icase = 1
    elseif(D .eq. 0._r8)then !       => 3 real roots, atleast 2 identical
       icase = 2
...
2   thesign = 1._r8
    ...
    ! case 3: D < 0
3   if(B.gt.0._r8)then
       thesign = -1._r8
    elseif(B.lt.0._r8)then
       thesign = 1._r8
    endif
    ...
    do kk = 1, 3
       k = kk-1
       y = 2._r8*Sqrt(-A/3._r8)*cos(phi + 120._r8*k*0.017453293_r8)
       x(kk) = ((y) - (p/3.d0))
       if(x(kk).gt.cubic) cubic = x(kk)
    enddo
    return
  ...
  function quadratic(a,b,c)
    ...
    if(abs(x) .lt. 1.e-6_r8)then
       ...
       quadratic = (-0.5_r8*b/a)*dum

       if(quadratic .lt. 0._r8)then
          quadratic = -b/a - quadratic
       endif

    else
       quad1 = ((-b)+sqrt((b*b)-(4._r8*a*c)))/   &
            (2._r8*a)
       quad2 = ((-b)-sqrt((b*b)-(4._r8*a*c)))/   &
            (2._r8*a)
  ...
  function mean_molecular_speed(T, MW)    ! in cm/s
    ...
    mean_molecular_speed = 1.455e4_r8 * sqrt(T/MW)
  ...
  function gas_diffusivity(T, P, MW, Vm)  ! in cm^2/s
    ...
    gas_diffusivity = (1.0e-3_r8 * T**1.75_r8 * sqrt(1._r8/MW + 0.035_r8))/   &
         (P * (Vm**third + 2.7189_r8)**2)
  ...
  function fuchs_sutugin(rkn,a)
    ...
    rnum  = 0.75_r8*a*(1._r8 + rkn)
    denom = rkn**2 + rkn + 0.283_r8*rkn*a + 0.75_r8*a
...
!          (aer(ibc_a,  jp,ibin)/dens_aer_mac(ibc_a  ))*kappa_nonelectro(ibc_a  ) )*aH2O_60/(1.0-aH2O_60)  ! RCE 5/20/2015
    ...
    tmpa = tmpa + tmpb * dens_water_aer * 1.0e-3_r8 * aH2O_60/(1.0_r8-aH2O_60)   ! RCE 1/6/2016 added 1.0e-3 factor

    aerosol_water_up = tmpa*1.e-9_r8
  ...
  function bin_molality_60(je,a_zsr)            ! TOUCH
    ...
    bin_molality_60 = 55.509_r8*xm/(1._r8 - xm)
...
!       * 1.0e-3 * aH2O/(1.0-min(ah2o,ah2o_max))                                             ! RCE 5/20/2015 - need 1.0e-3 factor
    ...
    tmpa = tmpa + tmpb * dens_water_aer * 1.0e-3_r8 * ah2o/(1.0_r8-min(ah2o,ah2o_max))

    aerosol_water = tmpa*1.e-9_r8	! kg(water)/m^3(air)

                 
    iclm_aer = 0 !BSINGH- THIS IS WRONG!!!
    jclm_aer = 0 !BSINGH- THIS IS WRONG!!!
    if(aerosol_water .le. 0.0_r8)then !BALLI- Commented out to avoid slow runtime.
...
44  format(a7, 2x, e11.3)
  ...
  function bin_molality(je,ibin,aH2O_a,b_zsr,a_zsr,aw_min)
    ...
    if(aw .lt. 0.97_r8)then
       ...
       bin_molality = 55.509_r8*xm/(1._r8 - xm)
