    ...
    do m = 1, nbins
      do l = 1, nspec(m) ! loop through species
         ...
         if (l <= nspec(m) ) then   ! species
            call rad_cnst_get_info_by_bin_spec(0, m, l, spec_name=fieldname(mm), spec_name_cw=fieldname_cw(mm))
         else  !number
            call rad_cnst_get_info_by_bin(0, m, num_name=fieldname(mm), num_name_cw=fieldname_cw(mm))
         end if
