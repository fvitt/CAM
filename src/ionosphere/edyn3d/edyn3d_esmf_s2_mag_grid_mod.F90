module edyn3d_esmf_s2_mag_grid_mod
...
contains
  ...
  subroutine edyn3d_esmf_s2_mag_grid_init
    ...
    print*,' '
    print*,'FVDBG.edyn3d_esmf_s2_mag_grid_init.. '
    ...
    vertloop: do k = 1, nz
       ...
       print*,'FVDBG.. ncells_hlat: ',ncells_hlat !,' nmlatS2_h - (k-1):',nmlatS2_h - (k-1)
       ...
       mag_s2_fdln_grid(k) = ESMF_GridCreate1PeriDim(  &
       ...
       ! S2 coordinates
       call ESMF_GridAddCoord(grid=mag_s2_fdln_grid(k),staggerloc=ESMF_STAGGERLOC_CENTER, rc=rc)
       call check_error(subname,'ESMF_GridAddCoord mag_s2_fdln_grid',rc)
       ...
       do nde = 0,localDECount-1
          ...
          call ESMF_GridGetCoord(mag_s2_fdln_grid(k), coordDim=1, localDE=nde, &
          ...
          call ESMF_GridGetCoord(mag_s2_fdln_grid(k), coordDim=2, localDE=nde, &
               computationalLBound=lbnd, computationalUBound=ubnd, &
               staggerloc=ESMF_STAGGERLOC_CENTER, farrayPtr=latcoord, rc=rc)
          call check_error(subname,'ESMF_GridGetCoord  S2',rc)
          latcoord = NOTSET

          print*,' S2 grid lat lbnd : ',lbnd
          print*,' S2 grid lat ubnd : ',ubnd
