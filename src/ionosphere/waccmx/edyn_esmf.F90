...
!!XXgoldyXX: ^ Eventually remove these items?
   ...
   public :: edyn_allocate_physvar
   public :: edyn_esmf_update

#ifdef WACCMX_EDYN_ESMF

   public :: edyn_allocate_esmf_field_array

   public :: phys_mesh
   public :: geo_grid

   public :: f_3desmf              ! array of ESMF fields
   ...
   !
   type(ESMF_Field) :: & ! 3d ESMF fields on physics mesh
        phys_ped,      & ! Pedersen conductivity
        phys_hal,      & ! Hall conductivity
        phys_zpot,     & ! geopotential height (cm)
        geo_scht,      & ! scale height (cm)
        geo_adotv1,    & ! ue1 (m/s)
        geo_adotv2       ! ue2 (m/s)
   type(ESMF_Field), allocatable :: f_3desmf(:) ! fields for regridding
   !
   ! 2d (i,j) ESMF fields on physics subdomains:
   !
   type(ESMF_Field) :: &   ! 2d ESMF fields on physics mesh
        geo_sini,      &   ! sin(I_m)
        geo_adota1,    &   ! d(1)**2/D
        geo_adota2,    &   ! d(2)**2/D
        geo_a1dta2,    &   ! (d(1) dot d(2)) /D
        geo_be3,       &   ! mag field strength (T)
        phys_efx, phys_kev ! amie fields
   !
   ...
   !
   ! 3d electric potential and electric field for mag to phys regridding:
   !
   type(ESMF_Field) :: mag_phi3d,  mag_ephi3d,  mag_elam3d,  mag_emz3d
   type(ESMF_Field) :: phys_phi3d, phys_ephi3d, phys_elam3d, phys_emz3d
...
!!XXgoldyXX: ^ debug only
   logical, parameter    :: debug = .false. ! set true for prints at each call
!!XXgoldyXX: v debug only
#else
...
contains
   ...
   subroutine edyn_allocate_physvar(physvar)
      real(r8), allocatable, intent(out) :: physvar(:,:)

      if (allocated(physvar)) then
         deallocate(physvar)
      end if
      allocate(physvar(pdim1s:pdim1e, pdim2s:pdim2e))
   end subroutine edyn_allocate_physvar

#ifdef WACCMX_EDYN_ESMF
   subroutine edyn_allocate_esmf_field_array(earray, num_fields)
      type(ESMF_Field), allocatable, intent(out) :: earray(:)
      integer,                       intent(in)  :: num_fields

      if (allocated(earray)) then
         deallocate(earray)
      end if
      allocate(earray(num_fields))
   end subroutine edyn_allocate_esmf_field_array
   ...
   subroutine edyn_create_physmesh(mesh_out)
      ...
      use ESMF,         only: ESMF_FILEFORMAT_ESMFMESH, ESMF_MeshCreate
...
!!XXgoldyXX: ^ remove?
   ...
   subroutine edyn_esmf_update
      ...
      !
      call edyn_esmf_create_physfield(phys_ped,    phys_mesh, 'PED',  nlev)
      call edyn_esmf_create_physfield(phys_hal,    phys_mesh, 'HAL',  nlev)
      call edyn_esmf_create_physfield(phys_zpot,   phys_mesh, 'ZPOT', nlev)
      !
      ! 3d fields on destination physics mesh for mag2phys:
      !
      call edyn_esmf_create_physfield(phys_phi3d,  phys_mesh, 'PHIG3D',   nlev)
      call edyn_esmf_create_physfield(phys_ephi3d, phys_mesh, 'EPHI3D',   nlev)
      call edyn_esmf_create_physfield(phys_elam3d, phys_mesh, 'ELAM3D',   nlev)
      call edyn_esmf_create_physfield(phys_emz3d,  phys_mesh, 'EMZ3D',    nlev)
      call edyn_esmf_create_physfield(phys_efx,    phys_mesh, 'GEFXAMIE', 0)
      call edyn_esmf_create_physfield(phys_kev,    phys_mesh, 'GKEVAMIE', 0)
      !
      ! 3d fields (inputs to edynamo) on geo grid for geo2mag:
      !
      call edyn_esmf_create_geofield(geo_adotv1, geo_grid, 'ADOTV1', nlev)
      call edyn_esmf_create_geofield(geo_adotv2, geo_grid, 'ADOTV2', nlev)
      !
      ! 2d fields on geo grid for geo2mag:
      !
      call edyn_esmf_create_geofield(geo_sini,   geo_grid, 'SINI',   0)
      call edyn_esmf_create_geofield(geo_adota1, geo_grid, 'ADOTA1', 0)
      call edyn_esmf_create_geofield(geo_adota2, geo_grid, 'ADOTA2', 0)
      call edyn_esmf_create_geofield(geo_a1dta2, geo_grid, 'A1DTA2', 0)
      call edyn_esmf_create_geofield(geo_be3,    geo_grid, 'BE3',    0)
      !
      ! Get 3d bounds of source geo field:
      !
      call ESMF_FieldGet(geo_adotv1, localDe=0, farrayPtr=fptr,               &
      ...
      !
      ! 3d fields on source mag grid for mag2phys:
      !
      ...
      !
      call ESMF_FieldGet(phys_phi3d, localDe=0, farrayPtr=fptr,               &
           computationalLBound=lbnd_destgeo,                                  &
           computationalUBound=ubnd_destgeo, rc=rc)
      call edyn_esmf_chkerr(subname, 'ESMF_FieldGet, geo_phi3d', rc)
      ...
      !
      call ESMF_FieldRegridStore(srcField=phys_ped, dstField=mag_ped,         &
      ...
      call edyn_esmf_chkerr(subname, 'ESMF_FieldRegridStore for 3D phys2mag', &
           rc)
      call ESMF_FieldSMMStore(phys_ped, mag_ped, routehandle_phys2mag,        &
      ...
      !
      call ESMF_FieldRegridStore(srcField=phys_efx, dstField=mag_sini,        &
           regridMethod=ESMF_REGRIDMETHOD_BILINEAR,                           &
           polemethod=ESMF_POLEMETHOD_ALLAVG,                                 &
      ...
      call edyn_esmf_chkerr(subname, 'ESMF_FieldRegridStore for 2D phys2mag', &
           rc)
      call ESMF_FieldSMMStore(phys_efx, mag_sini, routehandle_phys2mag_2d,    &
           factorList, factorIndexList, srcTermProcessing=smm_srctermproc,    &
           pipelineDepth=smm_pipelinedep, rc=rc)
      ...
      !
      call ESMF_FieldRegridStore(srcField=mag_phi3d, dstField=phys_phi3d,     &
           regridMethod=ESMF_REGRIDMETHOD_BILINEAR,                           &
           polemethod=ESMF_POLEMETHOD_ALLAVG,                                 &
      ...
      call edyn_esmf_chkerr(subname, 'ESMF_FieldRegridStore for 3D mag2phys', &
           rc)
      call ESMF_FieldSMMStore(mag_phi3d, phys_phi3d, routehandle_mag2phys,    &
           factorList, factorIndexList, srcTermProcessing=smm_srctermproc,    &
           pipelineDepth=smm_pipelinedep, rc=rc)
      ...
      call ESMF_FieldRegridStore(srcField=mag_efx, dstField=phys_efx,         &
           regridMethod=ESMF_REGRIDMETHOD_BILINEAR,                           &
           polemethod=ESMF_POLEMETHOD_ALLAVG,                                 &
      ...
      call edyn_esmf_chkerr(subname, 'ESMF_FieldRegridStore for 2D mag2phys', &
           rc)
      call ESMF_FieldSMMStore(mag_efx, phys_efx, routehandle_mag2phys_2d,     &
           factorList, factorIndexList, srcTermProcessing=smm_srctermproc,    &
           pipelineDepth=smm_pipelinedep, rc=rc)
      call edyn_esmf_chkerr(subname, 'ESMF_FieldSMMStore for 3D mag2phys', rc)
      !
      ...
      !
      call ESMF_FieldRegridStore(srcField=geo_adotv1, dstField=mag_ped,       &
           regridMethod=ESMF_REGRIDMETHOD_BILINEAR,                           &
           polemethod=ESMF_POLEMETHOD_ALLAVG,                                 &
           routeHandle=routehandle_geo2mag, factorIndexList=factorIndexList,  &
           factorList=factorList, srcTermProcessing=smm_srctermproc,          &
           pipelineDepth=smm_pipelinedep, rc=rc)
      call edyn_esmf_chkerr(subname, 'ESMF_FieldRegridStore for 3D geo2mag',  &
           rc)
      call ESMF_FieldSMMStore(phys_ped, mag_ped, routehandle_geo2mag,         &
           factorList, factorIndexList, srcTermProcessing=smm_srctermproc,    &
           pipelineDepth=smm_pipelinedep, rc=rc)
      ...
      !
      call ESMF_FieldRegridStore(srcField=geo_sini, dstField=mag_sini,        &
           regridMethod=ESMF_REGRIDMETHOD_BILINEAR,                           &
           polemethod=ESMF_POLEMETHOD_ALLAVG,                                 &
   ...
   subroutine create_mag_grid(grid_out, srcdes)
      ...
      call edyn_esmf_chkerr(subname,                                          &
           'ESMF_GridGetCoord for longitude coords', rc)
      ...
      call edyn_esmf_chkerr(subname,                                          &
           'ESMF_GridGetCoord for latitude coords', rc)
   ...
   subroutine edyn_esmf_create_physfield(field, mesh, name, nlev)
      ...
      if (nlev > 0) then
         ...
         field = ESMF_FieldCreate(mesh, arrayspec, gridToFieldMap=(/1,3/),    &
              ungriddedLBound=(/1/), ungriddedUBound=(/nlev/), rc=rc)
         !!XXgoldyXX: Use meshloc = ESMF_MESHLOC_ELEMENT?
         !!XXgoldyXX: Misleading optional argument, gridToFieldMap?
   ...
   !-----------------------------------------------------------------------
   subroutine edyn_esmf_set3d_geo(fields, fnames, fdata, nf,                  &
        ilev0, ilev1, ilon0, ilon1, ilat0, ilat1)
      !
      ...
      real(r8), intent(in) :: fdata(ilev0:ilev1,ilon0:ilon1,ilat0:ilat1,nf)
      !
      ! Local:
      ...
      if (nf > mxf) then
         if (masterproc) then
            write(iulog,"('>>> ',2a,i4,a,i4)") subname,                       &
                 ': nf cannot be greater than mxf: nf=', nf, ' mxf=', mxf
         end if
         call endrun(subname//': nf > mxf')
      end if
      !
      ! This routine is called every timestep from dynamo_input
      !
      ! Get array bounds:
      ...
      !
      ! Do the allocation:
      allocate(ftmp(lbnd(1):ubnd(1),lbnd(2):ubnd(2),lbnd(3):ubnd(3),mxf),     &
           stat=istat)
      if (istat /= 0) then
         write(iulog,"('>>> ',a,': error allocating ftmp')") subname
         call endrun(subname//': allocating ftmp')
      end if
      !
      ! Fields loop:
      do n=1, nf
            ...
            if (j /= jspole .and. j /= jnpole) then ! interior latitudes (not poles)
               do i=lbnd(1), ubnd(1)    ! lon
                  ...
                  do k=lbnd(3), ubnd(3)  ! lev
                     ftmp(i,j,k,n) = fdata(k,ii,j,n)
                  end do ! lev
               end do   ! lon
            end if     ! poles or interior
         end do       ! lat
      end do         ! n=1,nf
      !
   ...
   !-----------------------------------------------------------------------
   subroutine edyn_esmf_set2d_geo(field, grid, fname, fdata,                  &
      ...
      do j = lbnd(2), ubnd(2)
         ...
         end if       ! interior latitudes only
      end do

      if (debug .and. masterproc) then
         write(iulog,"(4a,2i4,a,2i4,a,2e12.4)") subname, 'field',             &
              trim(fname), ': lon bnds=', lbnd(1), ubnd(1),                   &
              ', lat bnds=', lbnd(2), ubnd(2),      &
              ', 2d mnmx=', minval(fptr(:,:)), maxval(fptr(:,:))
        end if

  end subroutine edyn_esmf_set2d_geo
  !-----------------------------------------------------------------------
  subroutine edyn_esmf_set3d_mag(fields, fnames, fdata, nf,                   &
       ilev0, ilev1, ilon0, ilon1, ilat0, ilat1)
     !
     ...
     !
     integer,          intent(in) :: ilev0, ilev1, ilon0, ilon1, ilat0, ilat1
     real(r8),         intent(in) :: fdata(ilon0:ilon1,ilat0:ilat1,ilev0:ilev1,nf)
     !
     ! Local:
     integer                       :: i, j, k, rc, n
     integer                       :: lbnd(3), ubnd(3) ! 3d field bounds
  ...
  !-----------------------------------------------------------------------
!
  subroutine edyn_esmf_set2d_mag(fields, fnames, fdata, nf,                   &
       ilon0, ilon1, ilat0, ilat1)
     !
     ...
     !
     integer,          intent(in) :: ilon0, ilon1, ilat0, ilat1
     real(r8),         intent(in) :: fdata(ilon0:ilon1,ilat0:ilat1,nf)
     !
     ! Local:
     integer                       :: i, j, rc, n
  ...
  !-----------------------------------------------------------------------
  subroutine edyn_esmf_set3d_phys(fields, fnames, fdata, nf,                  &
       ilev0, ilev1, icol0, icol1, iblk0, iblk1)
     !
     ! Set values of a 3d ESMF field on physics grid, prior to physics to
     ! magnetic grid transformation.
     !
     ! Args:
     integer,          intent(in) :: nf
     type(ESMF_Field), intent(in) :: fields(nf) ! esmf fields on phys grid
     character(len=*), intent(in) :: fnames(nf) ! field names
     !
     ! fdata is input data on model subdomains:
     !
     integer,          intent(in) :: ilev0, ilev1, icol0, icol1, iblk0, iblk1
     real(r8),         intent(in) :: fdata(icol0:icol1,iblk0:iblk1,ilev0:ilev1,nf)
     !
     ! Local:
     integer                       :: i, j, k, rc, n
     integer                       :: lbnd(3), ubnd(3) ! 3d field bounds
  ...
  !-----------------------------------------------------------------------
  !
  subroutine edyn_esmf_set2d_phys(fields, fnames, fdata, nf,                  &
       icol0, icol1, iblk0, iblk1)
     !
     ! Set values of a 2d ESMF field on magnetic grid, prior to magnetic to
     ! physics grid transformation.
     !
     ...
     ! fdata is input data on model subdomains:
     !
     integer,          intent(in) :: icol0, icol1, iblk0, iblk1
     real(r8),         intent(in) :: fdata(icol0:icol1,iblk0:iblk1,nf)
     !
     ! Local:
     integer                       :: i, j, rc, n
     integer                       :: lbnd(2), ubnd(2) ! 2d field bounds
  ...
  subroutine edyn_esmf_get_3dfield(field, fptr, name)
     ...
     ! Args:
     type(ESMF_field),  intent(in)  :: field
     real(r8), pointer, intent(out) :: fptr(:,:,:)
     character(len=*),  intent(in)  :: name
     !
     ! Local:
