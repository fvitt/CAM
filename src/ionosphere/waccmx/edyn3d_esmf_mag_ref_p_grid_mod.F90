module edyn3d_esmf_mag_ref_p_grid_mod
  ...
  private
  public :: edyn3d_esmf_mag_ref_p_grid_init
  public :: edyn3d_esmf_mag_ref_p_grid_destroy
  public :: mag_ref_p_fdln_grid

  type(ESMF_Grid), protected :: mag_ref_p_fdln_grid
...
contains
  ...
  subroutine edyn3d_esmf_mag_ref_p_grid_init()
    ...
    character(len=*), parameter :: subname = 'edyn3d_esmf_mag_ref_p_grid_init'
    ...
    ! number of global lat grid cells for ref level (1 DE straddles the equator)
    ...
    do i = 1,klat_sz-1
       ...
       j0 = mlat0_task(ii-1)
       j1 = mlat1_task(ii-1)
       latCellsPerDE(i) = count( npts_p(j0:j1)>=1)
    end do
    ...
    latCellsPerDE(i) = 2*count( npts_p(j0:j1)>=1 ) -1 ! each side of the equator (only 1 at the equator)
    ...
    mag_ref_p_fdln_grid = ESMF_GridCreate1PeriDim(  &
    ...
    ! P coordinates
    call ESMF_GridAddCoord(grid=mag_ref_p_fdln_grid,staggerloc=ESMF_STAGGERLOC_CENTER, rc=rc)
    call check_error(subname,'ESMF_GridAddCoord mag_ref_p_fdln_grid',rc)

    do nde = 0,localDECount-1

       ! set P coordinates
       ! geographic longitudes
       call ESMF_GridGetCoord(mag_ref_p_fdln_grid, coordDim=1, localDE=nde, &
            computationalLBound=lbnd, computationalUBound=ubnd, &
            staggerloc=ESMF_STAGGERLOC_CENTER, farrayPtr=loncoord, rc=rc)
       call check_error(subname,'ESMF_GridGetCoord P logitudes',rc)
       loncoord = NOTSET

       ! geographic latitudes
       call ESMF_GridGetCoord(mag_ref_p_fdln_grid, coordDim=2, localDE=nde, &
            computationalLBound=lbnd, computationalUBound=ubnd, &
       ...
       do i = lbnd(1),ubnd(1)
          do j = lbnd(2),ubnd(2)
             ...
             loncoord(i,j) = glon_p(1,isn,jj,i)
             latcoord(i,j) = glat_p(1,isn,jj,i)
          end do
       end do
  ...
  end subroutine edyn3d_esmf_mag_ref_p_grid_init
  ...
  subroutine edyn3d_esmf_mag_ref_p_grid_destroy()
    ...
    call ESMF_GridDestroy(mag_ref_p_fdln_grid, rc=rc)
    call check_error(subname, 'ESMF_GridDestroy', rc)

  end subroutine edyn3d_esmf_mag_ref_p_grid_destroy

end module edyn3d_esmf_mag_ref_p_grid_mod
