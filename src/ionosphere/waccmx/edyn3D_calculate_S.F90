!
!       subroutine edyn3D_calculate_S(fline_p,fline_s1,fline_r)
       subroutine edyn3D_calculate_S(fline_p,fline_s1,fline_s2,fline_r)
!
...
!
       ...
       use edyn3D_params,    only:nmlat_h,nmlatS2_h,J3LB,val_fill,nhgt_fix,hgt_fix,use_stabil
       use edyn3D_mpi,       only:mlon0_p,mlon1_p
       use edyn3D_fieldline, only:fieldline_p,fieldline_s1,fieldline_s2,fieldline_r
!       use edyn3D_fieldline, only:fieldline_p,fieldline_s1,fieldline_s2,fieldline_r,fline_s2
...
!
       ...
       type(fieldline_r),dimension(mlon0_p-1:mlon1_p+1,nmlat_h,2),intent(in) :: fline_r
!
       integer :: isn,i,j,k,nmax,isn1,isn2,im
       real(r8) :: tmp1, tmp2, sum1(nhgt_fix)
!
...
!
          do k = 1,fline_p(i,j,isn)%npts
            fline_p(i,j,isn)%S(k) = fline_s2(i,j,isn)%M2(k)*fline_s2(i,j,isn)%Je2D(k)
          enddo
!
...
! Q from H. Wu: this is not consistent with locations other than pole
!
...
!
       ...
       end subroutine edyn3D_calculate_S
