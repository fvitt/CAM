...
! Change the following parameters if adding more variables
  ...
  integer :: nlatp2=0, nlonp1=0, nlat=0, nlon=0 ! Needed for geographic conversions later

contains

  !-----------------------------------------------------------------------

  subroutine init_mage(mage_list)
    character(len=*),intent(in) :: mage_list(:)

    call init_coupling()

  end subroutine init_mage

  !-----------------------------------------------------------------------

  subroutine getmage(iyear, imo, iday, iutsec, sunlon, iprint,  &
    ...
    if (mytid .eq. 0) then
      write(*,'(A,I4,1X,I2.2,1X,I2.2,1X,I2,":",I2.2,":",I2.2)') "WCMX Coupling at ", &
  ...
  subroutine init_coupling()
    ...
    integer :: i,ierr
    character(len=*), parameter :: preface = 'mage_module::init_coupling : WACCMX send to REMIX : '
  ...
  subroutine mp_coupling()
    ...
    if (CplCommSize == 1) then
      ...
      select case (IAm(i))
        case (voltId)
          mixCplRank = i-1
          if (mytid .eq. 0) write(*,*) "W coupling to remix",mixCplRank
        case (gamId)
          if (mytid .eq. 0) write(*,*) "W not coupling to Gam yet"
        case (rcmId)
          if (mytid .eq. 0) write(*,*) "W not coupling to RCM yet"
        case (hidraNId)
          hidraNCplRank = i-1
          if (mytid .eq. 0) write(*,*) "W coupling to hidraN"
        case (hidraSId)
          hidraSCplRank = i-1
          if (mytid .eq. 0) write(*,*) "W coupling to hidraS"
        case (hidraId)
          hidraCplRank = i-1
          if (mytid .eq. 0) write(*,*) "W coupling to hidra"
        case (myAppId,myAppId*100,myAppId*100+1)
          if (mytid .eq. 0 .and. IAm(i) .eq. myAppId*100+1) write(*,*) "W is W", i-1
          IAm(i) = myAppId
        case default
          if (mytid .eq. 0) write(*,*) "W does not know about", &
...
!-----------------------------------------------------------------------
  ...
  subroutine export_mage()
    ...
    real(r8),dimension(:,:,:,:), allocatable :: hidraout
    integer :: nreq,i
    ...
    hidra_prep = .false.
    nreq = 0
!    if (mytid == 0) write(*,*) "W Starting Export Prep"
    ...
    do i=1,CplCommSize
      ...
      select case (IAm(i))
        ...
        case default
          if (IAm(i) .eq. 0) cycle
          if (mytid == 0) &
            write(*,*) "W does not know about this Coupling ID: ", &
                 IAm(i)
      end select
    enddo
...
! Export the data
    if (mytid .eq. 0) then
!      write(*,*) "WCMX Waiting to export1:",mixCplRank,(myAppId+voltId)*100,nlatp2*nlonp1*nmixoutgeo
...
!      write(*,*) "WCMX Waiting to export2:",mixCplRank,(myAppId+voltId)*100,nmlat*nmlonp1*nmixoutapex
!      write(*,*) "WCMX export3: ",shape(avar2d)
      if ( nmixoutapex .ne. 0) then
...
! Prepare data for export
      ...
      if (mytid .eq. 0) then
!        write(*,*) "WCMX: Start Preparing Export"
...
!        if (mytid .eq. 0) write(*,*) "WCMX: Done Preparing Export"

      end subroutine prep_export_remix

!-----------------------------------------------------------------------

    subroutine prep_export_hidra(hidraout)
        real(r8),dimension(:,:,:,:), allocatable :: hidraout
    end subroutine prep_export_hidra

    subroutine export_hidra(hidra_out,commId,hidraCplRank)
        real(r8),dimension(:,:,:,:) :: hidra_out
        integer :: commId,hidraCplRank
