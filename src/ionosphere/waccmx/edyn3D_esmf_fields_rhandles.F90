module edyn3D_esmf_fields_rhandles
  ...
  use edyn3D_mpi, only: mytid, tasks, ntask
  ...
  public :: rh_mag2plus_s1

  public :: edyn3D_esmf_fields_rhandles_init

  type(ESMF_Field) :: physFieldSrc
  type(ESMF_Field), allocatable :: magFieldDes_s1(:) ! nz vertical mag field levels
  ...
  type(ESMF_RouteHandle), allocatable :: rh_mag2plus_s1(:)
...
contains
  ...
  subroutine edyn3D_esmf_fields_rhandles_init
    ...
    integer :: lbnd(1), ubnd(1), i,j,k, isn, n, jj
    integer :: nmlat
    ...
    oplusFieldDes = ESMF_FieldCreate(oplusGrid, arrayspec, staggerloc=ESMF_STAGGERLOC_CENTER, &
         name="oplusFieldDes", ungriddedLBound=(/1/), ungriddedUBound=(/mag2opls_nflds/), rc=rc)
    call check_error(subname,'ESMF_FieldCreate oplusFeildDes',rc)
    ...
    do k = 1,nz
       ...
       if (mytid<ntask) then
          ...
          call check_error(subname,'ESMF_GridGetCoord mag_grid_s1 lon',rc)
          ...
          call check_error(subname,'ESMF_GridGetCoord mag_grid_s1 lat',rc)
       ...
       magFieldDes_s1(k) = ESMF_FieldCreate( grid=mag_grid_s1(k), typekind=ESMF_TYPEKIND_R8, &
            ungriddedLBound=(/1/), ungriddedUBound=(/phys2mag_nflds/), rc=rc)
    ...
    do k = 1,nz
       ...
       if (mytid<ntask) then
          ...
          call check_error(subname,'ESMF_GridGetCoord mag_grid_s2 lon',rc)
          ...
          call check_error(subname,'ESMF_GridGetCoord mag_grid_s2 lat',rc)
       ...
       magFieldDes_s2(k) = ESMF_FieldCreate( grid=mag_grid_s2(k), typekind=ESMF_TYPEKIND_R8, &
            ungriddedLBound=(/1/), ungriddedUBound=(/phys2mag_nflds/), rc=rc)
    ...
    do k = 1,nz
       ...
       ! src mag s1 grid
       nmlat = (nmlat_h - (k-1))*2 - 1 ! 1 point at equator
       nmlats_task(1) = nmlat

       mag_grid_src(k) = ESMF_GridCreate1PeriDim( &
            countsPerDEDim1=nmlons_task, countsPerDEDim2=nmlats_task, &
            coordDep1=(/1, 2/), coordDep2=(/1, 2/), &
            petmap=petmap, indexflag=ESMF_INDEX_GLOBAL, rc=rc)
       call check_error(subname,'ESMF_GridCreate1PeriDim mag_grid_src',rc)

       call ESMF_GridAddCoord(grid=mag_grid_src(k),staggerloc=ESMF_STAGGERLOC_CENTER, rc=rc)
       call check_error(subname,'ESMF_GridAddCoord mag_grid_src',rc)

       if (mytid<ntask) then
          !
          ! Get pointer and set mag grid geographic longitude coordinates:
          !
          call ESMF_GridGetCoord(grid=mag_grid_src(k), coordDim=1, farrayPtr=fptr2d, rc=rc)
          call check_error(subname,'ESMF_GridGetCoord mag_grid_src lon',rc)
          ...
          ! Get pointer and set mag grid geographic latitdue coordinates:
          !
          call ESMF_GridGetCoord(grid=mag_grid_src(k), coordDim=2, farrayPtr=fptr2d, rc=rc)
          call check_error(subname,'ESMF_GridGetCoord mag_grid_src lat',rc)
       ...
       magFieldSrc_s1(k) = ESMF_FieldCreate( grid=mag_grid_src(k), typekind=ESMF_TYPEKIND_R8, &
    ...
    do k = 1,nz
       ...
       call ESMF_FieldRegridStore( &
            ...
            factorList=factorList, srcTermProcessing=smm_srctermproc,          &
            pipelineDepth=smm_pipelinedep, rc=rc)
       call check_error(subname,'FieldRegridStore rh_mag2plus_s1(k) route handle',rc)
