module edyn3d_esmf_p_mag_grid_mod
  ...
  private
  public :: edyn3d_esmf_p_mag_grid_init
  public :: edyn3d_esmf_p_mag_grid_destroy
  public :: mag_p_fdln_grid

  type(ESMF_Grid), allocatable, protected :: mag_p_fdln_grid(:)
...
contains
  ...
  !-----------------------------------------------------------------------------
  !-----------------------------------------------------------------------------
  subroutine edyn3d_esmf_p_mag_grid_init()
    ...
    character(len=*), parameter :: subname = 'edyn3d_esmf_p_mag_grid_init'
    real(r8), parameter :: NOTSET = -huge(1._r8)

    allocate(mag_p_fdln_grid(nz), stat=astat)
    if (astat/=0) then
       call endrun(subname//' : not able to allocate mag_p_fdln_grid array')
    end if
    ...
    vertloop: do k = 1, nz
       ...
       do i = 1,klat_sz-1
          ...
          j0 = mlat0_task(ii-1)
          j1 = mlat1_task(ii-1)
          latCellsPerDE(i) = count( npts_p(j0:j1)>=k )
       end do
       ...
       latCellsPerDE(i) = 2*count( npts_p(j0:j1)>=k ) -1 ! each side of the equator (only 1 at the equator)
       ...
       mag_p_fdln_grid(k) = ESMF_GridCreate1PeriDim(  &
       ...
       ! P coordinates
       call ESMF_GridAddCoord(grid=mag_p_fdln_grid(k),staggerloc=ESMF_STAGGERLOC_CENTER, rc=rc)
       call check_error(subname,'ESMF_GridAddCoord mag_p_fdln_grid',rc)

       do nde = 0,localDECount-1

          ! set P coordinates
          ! geographic longitudes
          call ESMF_GridGetCoord(mag_p_fdln_grid(k), coordDim=1, localDE=nde, &
               computationalLBound=lbnd, computationalUBound=ubnd, &
               staggerloc=ESMF_STAGGERLOC_CENTER, farrayPtr=loncoord, rc=rc)
          call check_error(subname,'ESMF_GridGetCoord P longitudes',rc)
          loncoord = NOTSET

          ! geographic latitudes
          call ESMF_GridGetCoord(mag_p_fdln_grid(k), coordDim=2, localDE=nde, &
               computationalLBound=lbnd, computationalUBound=ubnd, &
               staggerloc=ESMF_STAGGERLOC_CENTER, farrayPtr=latcoord, rc=rc)
          call check_error(subname,'ESMF_GridGetCoord P latitudes',rc)
          latcoord = NOTSET
          ...
          do i = lbnd(1),ubnd(1)
             do j = lbnd(2),ubnd(2)
                ...
                loncoord(i,j) = glon_p(k,isn,jj,i)
                latcoord(i,j) = glat_p(k,isn,jj,i)
             end do
          end do
  ...
  !-----------------------------------------------------------------------------
  !-----------------------------------------------------------------------------
  subroutine edyn3d_esmf_p_mag_grid_destroy()
    use params_module, only: nz=>nhgt_fix

    integer :: k, rc
    character(len=*), parameter :: subname = 'edyn3d_esmf_p_mag_grid_destroy'

    do k = 1, nz
       call ESMF_GridDestroy(mag_p_fdln_grid(k), rc=rc)
       call check_error(subname, 'ESMF_GridDestroy', rc)
    end do

    deallocate(mag_p_fdln_grid)

  end subroutine edyn3d_esmf_p_mag_grid_destroy

end module edyn3d_esmf_p_mag_grid_mod
