...
!
  ...
  contains
!-----------------------------------------------------------------------
  subroutine dynamo(tn,un,vn,wn,zpot,ped,hall,ui,vi,wi,eeex,eeey,eeez, &
...
!
    real(r8),intent(inout),dimension(lev0:lev1,lon0:lon1,lat0:lat1) :: &
      ...
      eeex,         & ! zonal ion drift (cm/s)
      eeey,         & ! meridional ion drift (cm/s)
      eeez,         & ! meridional ion drift (cm/s)
...
!
    ...
    call ionvel(zpot,ui,vi,wi,eeex,eeey,eeez)
...
!-----------------------------------------------------------------------
  subroutine ionvel(z,ui,vi,wi,eeex,eeey,eeez)
!
...
!
      do k=lev0,lev1
        do i=lon0,lon1
          ...
          eeex(k,i,j) = eex(k,i)*1.e6_r8/bmod(ii,j)
          eeey(k,i,j) = eey(k,i)*1.e6_r8/bmod(ii,j)
          eeez(k,i,j) = eez(k,i)*1.e6_r8/bmod(ii,j)
          !if ((j < 6 .or.  j>90)) then
          !eeex(k,i,j) = eeex(k,i,j)*0.75_r8
          !eeey(k,i,j) = eeey(k,i,j)*0.75_r8
          !eeez(k,i,j) = eeez(k,i,j)*0.75_r8
          !endif
        enddo ! i=lon0,lon1
      enddo ! k=lev0,lev1
...
! Output ion drifts in cm/s for oplus_xport call from dpie_coupling:
      do i=lon0,lon1
        ...
        eeex(:,i,j) = eeex(:,i,j)*100._r8
        eeey(:,i,j) = eeey(:,i,j)*100._r8
        eeez(:,i,j) = eeez(:,i,j)*100._r8
      enddo
    enddo ! j=lat0,lat1 
