!------------------------------------------------------------------------------
! Provides methods for mapping from regular longitude / latitude grid to
...
contains
  ...
  subroutine esmf_lonlat2phys_init()
    ...
    ! route handle (interpolation weights)
    ...
    call check_esmf_error(rc, subname//'ESMF_FieldRegridStore routehandle ERROR')

  end subroutine esmf_lonlat2phys_init

  !------------------------------------------------------------------------------
  !------------------------------------------------------------------------------
  subroutine esmf_lonlat2phys_regrid(lonlatflds, physflds)
    use esmf_lonlat_grid_mod, only: lon_beg,lon_end,lat_beg,lat_end
    use ppgrid, only: pcols, pver, begchunk, endchunk
    use phys_grid, only: get_ncols_p

    type(fields_bundle_t), intent(in) :: lonlatflds(nflds)
    type(fields_bundle_t), intent(inout) :: physflds(nflds)

    integer :: i, ichnk, ncol, ifld, ilev, icol, rc
    real(ESMF_KIND_R8), pointer :: physptr(:,:,:)
    real(ESMF_KIND_R8), pointer :: lonlatptr(:,:,:,:)

    character(len=*), parameter :: subname = 'esmf_lonlat2phys_regrid: '
  ...
  !  print*,'FVDBG.esmf_lonlat2phys_regrid min max lonlatptr : ',minval(lonlatptr), maxval(lonlatptr)
  ...
  !  print*,'FVDBG.esmf_lonlat2phys_regrid min max physptr : ',minval(physptr), maxval(physptr)
    ...
    do ichnk = begchunk, endchunk
       ...
       do icol = 1,ncol
          ...
          do ifld = 1,nflds
             do ilev = 1,pver
                physflds(ifld)%fld(icol,ilev,ichnk) = physptr(ilev,ifld,i)
             end do
          end do
       end do
    end do

  end subroutine esmf_lonlat2phys_regrid
